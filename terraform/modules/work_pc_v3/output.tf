...
output "cockpit_url" {
  value = (var.debug_output == "true" && var.enable_web_console) ? "https://${local.worker_pc_ip}:9090" : null
  description = "Cockpit web console URL (only if enable_web_console=true)"
}

output "terminal_url" {
  value = (var.debug_output == "true" && var.enable_web_console) ? "http://${local.worker_pc_ip}" : null
  description = "Terminal URL (nginx redirect with basic auth)"
}

output "nginx_auth" {
  value = (var.debug_output == "true" && var.enable_web_console) ? "admin / ${random_string.ssh.result}" : null
  description = "Nginx basic auth credentials"
}

output "cockpit_login" {
  value = (var.debug_output == "true" && var.enable_web_console) ? "ubuntu / ${random_string.ssh.result}" : null
  description = "Cockpit login credentials"
}
