...
define terragrint_run
    ...
    @case "$(3)" in
        run)
            @commnand="terragrunt run-all  apply --parallelism=$(nproc --ignore=1) "
            ;;
        delete)
            @commnand="terragrunt run-all  destroy --parallelism=$(nproc --ignore=1) "
            ;;
        output)
            @commnand="terragrunt run-all  output --parallelism=$(nproc --ignore=1)"
            ;;
    esac
