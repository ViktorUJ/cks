...
## 0.35.1 (25.08.2026)
### Features
* Amazon EKS [self-study course](../tasks/eks/course/README.md) is added: 48 chapters (Part 0 - an optional foundation on AWS accounts, IAM, VPC, EC2 and tooling; Parts 1-9 - cluster architecture, nodes and compute, identity and security, storage, networking, observability, operations and troubleshooting) tied to the hands-on labs. Available in 8 languages: Russian (source), English, Spanish, French, German, Georgian, Traditional Chinese and Japanese.
* EKS labs [101-132](../docs/labs.MD#eks-labs) are added: 32 composite labs deployed through Terragrunt with automatic `check_result` validation and reference solutions, in the same 8 languages.
* EKS course reference guides are added: [Glossary](../tasks/eks/course/GLOSSARY.md), [Runbook](../tasks/eks/course/RUNBOOK.md), [ADRs](../tasks/eks/course/ADR.md), [Maturity scorecard](../tasks/eks/course/SCORECARD.md) and [Cost model](../tasks/eks/course/COST_MODEL.md), also in 8 languages.
### Improvements
* Istio [self-study course](../tasks/ica/course) and ICA labs [01-35](../tasks/ica/labs) are now fully localized: Georgian, Traditional Chinese and Japanese translations are added, bringing the course and labs to 8 languages (Russian, English, Spanish, French, German, Georgian, Traditional Chinese and Japanese).

## 0.34.1 (03.08.2026)
### Features
* CKA + CKAD joint [self-study course](../tasks/cka/course/README.md) is added: 58 chapters (Part 0 - an optional foundation on networking, DNS, TLS, containers, Linux, YAML and vim; Parts 1-10 - the exam program) tied to the hands-on labs. Every chapter is marked with the exam it belongs to (CKA / CKAD / both). Available in 8 languages: Russian (source), English, Spanish, French, German, Georgian, Traditional Chinese and Japanese.
* CKA/CKAD labs [101-125](../docs/labs.MD#cka-labs) are added: 25 composite labs in the exam style with the automatic `check_result` validation and the reference solutions, in the same 8 languages.
* Preparation route guides are added: [CKA program and labs](../tasks/cka/course/CKA.md) and [CKAD program and labs](../tasks/cka/course/CKAD.md) - the official exam domains with their weights mapped onto the chapters and the labs.
* [Course glossary](../tasks/cka/course/GLOSSARY.md) is added: all the terms of the course grouped by the chapters with the links.
### Improvements
...
* [Course contents](../tasks/cka/course/README.md) is extended: a `Practice` section with the links to the labs and to the CKA/CKAD mock exams, a `Which labs to choose` section (our labs for the exam preparation, Killercoda for a quick start) and a `What to read next` section with the further reading.

## 0.33.1 (20.07.2026)
### Features
* Lab 32 Istio.[gRPC: per-request load balancing, port naming, retries and timeouts](../tasks/ica/labs/32) is added.
* Lab 33 Istio.[Control plane performance & operations: discovery selectors, Sidecar scope, istiod golden signals, OPA Gatekeeper](../tasks/ica/labs/33) is added.
* Lab 34 Istio.[Hardening & threat model: STRICT mTLS, default-deny, egress control, RBAC on Istio CRDs, OPA Gatekeeper, NetworkPolicy](../tasks/ica/labs/34) is added.
* Lab 35 Istio.[Multicluster mesh: multi-primary, multi-network, shared CA, east-west gateway, cross-cluster discovery](../tasks/ica/labs/35) is added.
* Istio [self-study course](../tasks/ica/course) is added: 32 chapters (Part 1 - ICA exam prep, Part 2 - production best practices) tied to the hands-on labs. Available in 5 languages: Russian (source), English, Spanish, French and German.
### Improvements
...
* ICA labs [01-35](../tasks/ica/labs) are now translated into Spanish (`README_ES.MD`), French (`README_FR.MD`) and German (`README_DE.MD`) in addition to the existing Russian and English versions.

## 0.32.1 (03.07.2026)
### Features
* Lab 04 Istio.[Zero Trust: mTLS and AuthorizationPolicy](../tasks/ica/labs/04) is added.
* Lab 05 Istio.[Controlled Egress: ServiceEntry, Egress Gateway and Sidecar scope](../tasks/ica/labs/05) is added.
* Lab 06 Istio.[Load Balancing and Traffic Mirroring](../tasks/ica/labs/06) is added.
* Lab 07 Istio.[Helm install and Canary upgrade with revisions](../tasks/ica/labs/07) is added.
* Lab 08 Istio.[Observability: Prometheus, Jaeger and Kiali](../tasks/ica/labs/08) is added.
* Lab 09 Istio.[Advanced: Ambient mode](../tasks/ica/labs/09) is added.
* Lab 10 Istio.[Resilience: Timeout and Circuit Breaker](../tasks/ica/labs/10) is added.
* Lab 11 Istio.[End-user authentication: RequestAuthentication and JWT](../tasks/ica/labs/11) is added.
* Lab 12 Istio.[Troubleshooting: istioctl analyze, proxy-status, proxy-config](../tasks/ica/labs/12) is added.
* Lab 13 Istio.[Securing Edge Traffic with TLS](../tasks/ica/labs/13) is added.
* Lab 14 Istio.[Locality-aware Failover](../tasks/ica/labs/14) is added.
* Lab 15 Istio.[Installation & Configuration: customizing the Istio install (IstioOperator and MeshConfig)](../tasks/ica/labs/15) is added.
* Lab 16 Istio.[Kubernetes Gateway API: ingress routing with Gateway and HTTPRoute](../tasks/ica/labs/16) is added.
* Lab 17 Istio.[Rate Limiting: local rate limit with EnvoyFilter](../tasks/ica/labs/17) is added.
* Lab 18 Istio.[Telemetry API: access logs and distributed tracing](../tasks/ica/labs/18) is added.
* Lab 19 Istio.[Custom CA: plug your own root and intermediate CA into istiod](../tasks/ica/labs/19) is added.
* Lab 20 Istio.[mTLS migration: PERMISSIVE to STRICT with zero downtime](../tasks/ica/labs/20) is added.
* Lab 21 Istio.[Sidecar scoping: limit the proxy configuration scope](../tasks/ica/labs/21) is added.
* Lab 22 Istio.[TLS origination: initiate TLS from the mesh](../tasks/ica/labs/22) is added.
* Lab 23 Istio.[WasmPlugin: extend the data plane with WebAssembly](../tasks/ica/labs/23) is added.
* Lab 24 Istio.[Ambient: waypoint proxy and L7 authorization](../tasks/ica/labs/24) is added.
* Lab 25 Istio.[Progressive delivery with Flagger](../tasks/ica/labs/25) is added.
* Lab 26 Istio.[Dynamic CA: cert-manager and istio-csr](../tasks/ica/labs/26) is added.
* Lab 27 Istio.[EnvoyFilter and Lua: custom logic with an inline script](../tasks/ica/labs/27) is added.
* Lab 28 Istio.[TCP routing: routing non-HTTP traffic](../tasks/ica/labs/28) is added.
* Lab 29 Istio.[Ingress TLS: MUTUAL and PASSTHROUGH modes](../tasks/ica/labs/29) is added.
* Lab 30 Istio.[StatefulSets and headless services in the mesh](../tasks/ica/labs/30) is added.
* Lab 31 Istio.[Zero-downtime production migration: ingress-nginx to Istio Gateway](../tasks/ica/labs/31) is added.
### Improvements
* ICA labs [01](../tasks/ica/labs/01), [02](../tasks/ica/labs/02) and [03](../tasks/ica/labs/03) are reworked: valid `check_result` tests, reference solutions, per-lab prefixes and region `eu-central-1` / `t3` instances.
* Lab 07 Istio.[Helm install and Canary upgrade with revisions](../tasks/ica/labs/07): In-Place upgrade section is added.

## 0.31.1 (01.06.2026)
### Features
* [k8i kubectl plugin](https://github.com/ViktorUJ/kubectl-k8i) was released 
* Lab 03 EKS.[Application Autoscaling with KEDA and Prometheus based on rps](../tasks/eks/labs/03/README.MD) is added.
* Analize fature is added to [k8i kubectl plugin](https://github.com/ViktorUJ/kubectl-k8i) .shows all workloads running on a selected set of nodes. For each workload it displays the namespace, kind, name, pod count, and aggregated CPU/memory requests, limits, and usage.


## 0.30.1 (01.05.2026)
### Features
* K8s version **1.36** is added  to [k8s_self_managment_v2](../terraform/modules/k8s_self_managment_v2) module.  
* Lab 03 Istio.[Fault Injection and Retry](../tasks/ica/labs/03) is added.
* [ping_pong:debug](../docker/ping_pong/README.MD)  is updated (new aws cli , redis client)



## 0.29.1 (20.03.2026)
### Features
* K8s version **1.35** is added  to [k8s_self_managment_v2](../terraform/modules/k8s_self_managment_v2) module.  
* Article [Vibe Coding → Spec-Driven: How AI Development Found an Engineering Approach](../docs/articles/spec_driven_development/README.md) is added.
* Article [From Spot.io to Karpenter: Node Management Migration Experience in Kubernetes](../docs/articles/from_spot_io_to_karpenter/readme.MD) is added.
* Lab 01 Istio.[Installing Istio](../tasks/ica/labs/01) is added.
* Lab 02 Istio.[Dark Launch](../tasks/ica/labs/02) is added.

## 0.28.1 (17.02.2026)
### Features
* [ica mock exam 02](../tasks/ica/mock/02) is added.
* [ica mock exam 02 solutions](../tasks/ica/mock/02/worker/files/solutions) is added.
