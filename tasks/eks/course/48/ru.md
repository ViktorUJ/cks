...
### Границы курса: что здесь сознательно не разбирается

Курс держит одну тему - эксплуатацию EKS, - и всё, что уводит в сторону, отдано другим
источникам осознанно. Это не пробелы, а выбор границ. Ниже - что именно осталось за рамками
и куда идти за деталями.

| Тема | Почему за рамками | Куда идти |
|---|---|---|
| HashiCorp Vault глубже обзора: PKI и transit engine, установка в кластер, политики HCL, namespaces Vault | отдельный продукт со своей моделью эксплуатации, а не часть EKS; обзорный разбор Vault как слоя хранения секретов в курсе есть (глава 18) | документация Vault |
| Вендорские CI-пайплайны: готовые описания для GitHub Actions, GitLab CI и прочих | курс описывает GitOps как модель, а не синтаксис конкретного CI (глава 44) | документация вашей CI-системы |
| Мультиаккаунт и мультикластер на практике | разобраны как архитектура (глава 32), воспроизводимой практики нет: нужны минимум два аккаунта AWS | документация AWS Organizations и EKS |
| Аудит и детект с GuardDuty на практике | механика описана (глава 21), практики нет: платный сервис, срабатывает не сразу | документация Amazon GuardDuty |
| Прикладная разработка и код сервисов, включая схемы данных | курс про платформу, а не про то, как писать приложение | профильные источники по разработке |
| Прикладные сервисы AWS за пределами кластера: RDS, очереди, кэши | упоминаются как потребители и как источник затрат, своей эксплуатации в курсе нет | документация соответствующих сервисов AWS |
| Прогрессивная доставка глубже обзора: Argo Rollouts, Flagger | названы и разграничены с blue/green кластеров (глава 44), своей главы не имеют | документация Argo Rollouts и Flagger |
| Windows-ноды | упоминаются только там, где меняют механику: ограничения Pod Identity, типы access entry | документация EKS по Windows-нодам |
| Managed-возможность EKS для Argo CD в виде практики | разобрана в тексте (глава 44), лабы не будет: аутентификация идёт только через AWS Identity Center, а он требует AWS Organizations, и в личном аккаунте это барьер | документация EKS и AWS Identity Center |

Список границ - это не список недоделок. Каждая строка выше - решение о том, где кончается
эксплуатация EKS и начинается чужая предметная область. Если тема нужна вам прямо сейчас, курс
даёт достаточно контекста, чтобы читать профильную документацию не с нуля, а с понимания,
