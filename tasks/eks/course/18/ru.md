...
### HashiCorp Vault: тот же слой 2, но хранилище не от AWS

До сих пор в роли хранилища выступали Secrets Manager и SSM Parameter Store, но слой 2 не
привязан к AWS. Vault занимает то же место в схеме и приходит в кластер по одной из трёх
причин: он уже стоит в компании и обслуживает не только EKS, нужны **динамические секреты**
(AWS secrets engine выдаёт временные креды IAM, database engine - короткоживущего пользователя
БД под конкретный запрос), либо нужен единый источник для мультиоблака и своего дата-центра.

Аутентификация пода в Vault опирается на ту же механику, что и глава 16. Kubernetes auth
method проверяет токен ServiceAccount через `TokenReview` в API кластера; JWT/OIDC auth
проверяет проектированный токен по OIDC-issuer кластера, не обращаясь к API; AWS IAM auth
принимает подписанный запрос к `sts:GetCallerIdentity` - то есть узнаёт роль IRSA или Pod
Identity. Первый вариант проще, третий естественнее ложится на уже настроенный IRSA.

Доставка секрета в под - четыре способа, из которых два вы уже знаете:

- **Vault Agent Injector** - mutating webhook подставляет в под sidecar или init-контейнер,
  который логинится в Vault и пишет секрет в общий `emptyDir`; включается аннотациями
  `vault.hashicorp.com/agent-inject` и `vault.hashicorp.com/role`. В etcd не попадает ничего.
- **Vault Secrets Operator** - контроллер с CRD (`VaultStaticSecret`, `VaultDynamicSecret`,
  `VaultAuth`), который синхронизирует значение в нативный `Secret`. Это ровно модель ESO, со
  всеми её свойствами из таблицы выше.
- **ESO с провайдером Vault** - тот же оператор из 18.5, только `SecretStore` смотрит не в
  Secrets Manager, а в Vault. Удобно, когда часть секретов в AWS, часть в Vault.
- **Secrets Store CSI Driver с провайдером Vault** - монтирование файлами, как в 18.6.

Цена такая же честная, как в главе 8 про смену CNI: хранилище становится вашим. Свой Vault -
это HA-кластер со своим storage backend, unseal и recovery keys, обновления, бэкап и аудит; в
AWS его обычно разворачивают с auto-unseal через KMS (`seal "awskms"`), чтобы не хранить
unseal-ключи у людей. Managed-вариант от вендора снимает часть этой работы, но не
ответственность за политики и роли. Отдельный операционный нюанс: обращения к секретам видны в
audit device Vault, а не в CloudTrail, поэтому расследование доступа идёт по двум журналам
(глава 21). Слой 1 при этом никуда не девается: если секрет всё же синхронизирован в `Secret`,
он лежит в etcd и защищён KMS-шифрованием из 18.3.

## 18.8. Ротация: пароль к базе сменился

Ночью сработала ротация секрета базы. Утром картина: часть подов работает, часть валится с
ошибкой аутентификации, а в Secrets Manager лежит правильный новый пароль. Значение в AWS
обновилось мгновенно, но до приложения оно идёт через цепочку из четырёх звеньев, и застрять
может в любом.

| Звено | Что определяет задержку | Симптом при неверной настройке |
|---|---|---|
| Хранилище | стратегия ротации и момент смены пароля в БД | окно, когда пароль в БД новый, а читатели ещё со старым |
| Синхронизация в кластер | `refreshInterval` у ESO, rotation reconciler у CSI | `Secret` или файл в томе со старым значением |
| Как приложение получает значение | env против тома или файла | env не меняется никогда, том обновляется |
| Соединения к базе | пул соединений и логика реконнекта | пул живёт со старыми кредами до перезапуска |

**Звено 1: как ротирует Secrets Manager.** Ротацией управляет функция-ротатор, а версии
секрета помечены метками: `AWSCURRENT` читают все по умолчанию, `AWSPENDING` - новое значение
на проверке, `AWSPREVIOUS` - предыдущее. Стратегий две, и выбор прямо влияет на доступность.
При **single user** меняется пароль одного пользователя: открытые соединения не рвутся, но
между сменой пароля в БД и обновлением секрета есть короткий промежуток, когда попытка
подключиться со свежепрочитанными кредами может получить отказ. AWS считает эту стратегию
подходящей для большинства случаев, а риск закрывают повторами с экспоненциальной задержкой.
При **alternating users** в секрете два пользователя: ротатор клонирует исходного и далее
меняет пароли по очереди, поэтому приложение в любой момент ротации получает валидные креды, а
после неё работают оба набора. Цена - отдельный секрет с правами superuser (клонировать себя
пользователь обычно не может) и обязанность повторять изменения прав на клоне.

**Звено 2: как новое значение попадает в кластер.** У ESO это `refreshInterval` из 18.5: при
`0` секрет создаётся один раз и после ротации навсегда останется старым. У CSI Driver файлы в
томе обновляет отдельный rotation reconciler, и его нужно включить - без него том тоже
статичен. То есть «мы ротируем секреты» без настройки этого звена означает «мы меняем пароль
только в AWS».

**Звено 3: как значение видит процесс.** Переменные окружения задаются при старте контейнера и
**не обновляются никогда**, даже когда `Secret` уже новый. Значение из тома kubelet обновляет
сам, но приложение должно перечитать файл, а не держать пароль в памяти с момента запуска.
Отсюда два рабочих подхода: перезапуск пода по изменению секрета (Reloader из 18.5) либо
чтение из файла с реакцией на его изменение.

**Звено 4: соединения.** Даже перечитав пароль, приложение продолжит пользоваться уже открытым
пулом. Правильное поведение - при ошибке аутентификации перечитать креды и пересоздать
соединение с повтором и задержкой, а не падать в `CrashLoopBackOff` и не ждать ручного
рестарта.

**Как убрать проблему целиком.** Ротация пароля - это управление тем, чего лучше бы не было.
Для RDS и Aurora есть **IAM database authentication**: вместо пароля приложение получает токен
через `aws rds generate-db-auth-token`, живущий по умолчанию 15 минут, а права выдаёт роль пода
через IRSA или Pod Identity (главы 16 и 17). Ротировать нечего - нет постоянного пароля.
Похожую идею дают динамические секреты Vault из 18.7: креды выдаются под запрос и истекают
сами. Если же пароль всё-таки нужен, ручную смену на проде делают по логике alternating users:
сначала завести второго пользователя, перевести нагрузку, потом отозвать первого - а не менять
пароль работающего пользователя «в лоб».

## 18.9. KMS и внешние хранилища вместе
...
## 18.10. Диагностика: секрет не появился или не обновился
...
- **Staging labels** - метки версий секрета в Secrets Manager: `AWSCURRENT` читается по
  умолчанию, `AWSPENDING` - значение на проверке при ротации, `AWSPREVIOUS` - предыдущее.
- **Стратегия ротации** - `single user` (меняется пароль одного пользователя, есть короткое
  окно риска отказов, закрывается повторами с задержкой) или `alternating users` (два
  пользователя по очереди, валидные креды в любой момент, нужен секрет с правами superuser).
- **IAM database authentication** - вход в RDS или Aurora по временному токену
  (`aws rds generate-db-auth-token`, по умолчанию 15 минут) вместо пароля; ротировать нечего.
- **HashiCorp Vault** - внешнее хранилище секретов не от AWS, занимающее то же место, что
  Secrets Manager: аутентификация пода через Kubernetes, JWT/OIDC или AWS IAM auth; доставка
  через Vault Agent Injector, Vault Secrets Operator, ESO или CSI Driver с провайдером Vault.
  Главное отличие - **динамические секреты** (временные креды IAM и БД по запросу), цена -
  эксплуатация самого Vault и отдельный audit device вместо CloudTrail.
...
- Ротация доходит до приложения через четыре звена: стратегия в хранилище, синхронизация в
  кластер (`refreshInterval` или rotation reconciler), способ чтения значения (env не
  обновляется никогда) и пул соединений. Радикальный выход - IAM database authentication для
  RDS или динамические секреты, где постоянного пароля нет вовсе.

## 18.14. Как это пригодится в реальной работе
...
18.10 - роль, объект конфигурации, права на секрет и его KMS-ключ. А знание, что ESO кладёт
секрет в etcd, а CSI без sync - нет, помогает выбрать инструмент под нужный след.

## 18.15. Вопросы для самопроверки
...
14. Какие три причины приводят в кластер Vault и чем вы за это платите в эксплуатации?
15. Чем Vault Agent Injector отличается от Vault Secrets Operator по следу в etcd?
16. Пароль базы ротировался, в Secrets Manager он новый, а часть подов валится с ошибкой
    аутентификации. Разберите цепочку из четырёх звеньев: где именно застряло значение?
17. Чем `single user` отличается от `alternating users` по доступности и что требует второй?
18. Почему приложение с паролем в переменной окружения не переживает ротацию и какие два
    способа это решают?
