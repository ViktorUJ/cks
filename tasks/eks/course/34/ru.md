...
### Свой стек логов: Loki и VictoriaLogs

За пределами AWS-сервисов есть два решения, которые часто ставят в кластер рядом с Grafana,
особенно если метрики уже смотрят там же (глава 33).

**Grafana Loki** устроен на одной идее: индексировать не сам текст, а только **метки** потока,
как в Prometheus. Логи сжимаются в чанки и лежат в объектном хранилище, то есть в S3, а индекс
остаётся маленьким - отсюда дешёвое хранение. Запросы на **LogQL**, знакомом по синтаксису
метрик. Отсюда же главная ловушка, симметричная кардинальности из главы 33: метки должны быть
низкокардинальными (namespace, приложение, контейнер), а `pod`, `request_id` или `trace_id` в
метках разносят индекс и производительность; для них предусмотрены structured metadata.
Собирать логи можно тем же Fluent Bit, а родной агент Loki теперь Grafana Alloy: Promtail в
него влит и снят с поддержки.

**VictoriaLogs** из той же экосистемы, что VictoriaMetrics: одна беззависимостная база логов,
которой не нужны ни заранее заданная схема, ни настройка индексов. Хранит данные колоночно на
диске, запросы на **LogsQL** с полнотекстовым поиском, приём - по множеству протоколов,
включая Elasticsearch bulk, Loki push, OTLP и syslog, так что менять агентов при переезде
обычно не требуется. Есть кластерная версия (`vlinsert`, `vlstorage`, `vlselect`) и оператор
для Kubernetes.

| Решение | Что индексирует | Запросы | Где лежат логи | Эксплуатация |
|---|---|---|---|---|
| CloudWatch Logs | всё, managed | Logs Insights | у AWS | нет |
| OpenSearch Service | полнотекстовый индекс | DSL, Dashboards | кластер OpenSearch | сайзинг и апгрейды кластера |
| Loki | только метки потока | LogQL | объектное хранилище (S3) | компоненты Loki и дисциплина метк |
| VictoriaLogs | схема не нужна | LogsQL | диски ваших нод | минимум компонентов, диски на вас |

Выбор обычно сводится к трём вопросам. Всё в AWS и нужен минимум эксплуатации - CloudWatch, а
архив в S3. Нужен тяжёлый полнотекстовый поиск и готовые дашборды - OpenSearch, понимая цену
отдельного кластера. Дашборды уже в Grafana и хочется дешёвое хранение в S3 - Loki, с оглядкой
на кардинальность метк. Хочется того же, но проще в эксплуатации и без объектного хранилища -
VictoriaLogs. Как и с метриками, свой стек не бесплатен: платите дисками, нодами и дежурством
вместо счёта от AWS (структура затрат - раздел 34.6 и глава 43).
...
Ещё две опции INPUT спасают сам сборщик при backpressure - когда назначение недоступно или
троттлит (например, CloudWatch API отвечает медленно или отдаёт лимит запросов). Без них Fluent
Bit копит непринятые записи в памяти, пухнет и ловит OOMKilled, унося вместе с собой все логи
ноды - ровно то, от чего он должен защищать. Опция `Mem_Buf_Limit` в INPUT `tail` ограничивает
память под буфер: при достижении лимита плагин перестаёт читать новые файлы, пока очередь не
разгрузится, - вместо роста до OOM. Опция `storage.type filesystem` уводит переполнение буфера
на диск ноды (нужен `storage.path` в секции `SERVICE`), а не держит всё в RAM: пиковый затор
переживается без потерь и без OOM. Вместе они превращают сбой отправки в замедление, а не в
падение агента и потерю логов.

Две опции конвейера прямо влияют на пригодность логов к разбору. Опция `multiline.parser` в
INPUT `tail` склеивает многострочные записи в одну: стектрейс Java или Python иначе прилетит
десятком отдельных строк, и в хранилище их не собрать обратно. Встроенные парсеры (`cri`,
`docker`, `go`, `java`, `python`) закрывают типовые случаи; `cri` собирает строки, разбитые
самим container runtime, прикладные ставят следом. Опция `Merge_Log On` в фильтре `kubernetes`
разбирает JSON-строку из поля `log` в отдельные поля записи: приложение, пишущее логи в JSON,
становится структурированным - по его полям можно фильтровать и искать, а не по тексту целиком.
...
- **INPUT / FILTER / OUTPUT** - три вида секций конвейера Fluent Bit: чтение, обработка,
  отправка.
- **Grafana Loki** - хранилище логов, индексирующее только метки потока; логи сжаты в чанки
  в объектном хранилище, запросы на LogQL. Метки должны быть низкокардинальными, для высокой
  кардинальности есть structured metadata; родной агент - Grafana Alloy (Promtail влит в него).
- **VictoriaLogs** - беззависимостная база логов без схемы и настройки индексов; колоночное
  хранение на диске, запросы на LogsQL, приём по протоколам Elasticsearch bulk, Loki push, OTLP
  и syslog; есть кластерный вариант (`vlinsert`, `vlstorage`, `vlselect`).
...
14. Что индексирует Loki и почему `pod` или `request_id` в метках - плохая идея?
15. Чем VictoriaLogs отличается от Loki по хранению и по требованиям к настройке?
16. Логи смотрят в Grafana, хранить надо дёшево и долго. Какие два варианта и чем платите?

## Практика

Лаба курса к этой теме: [лаба 115 - Логирование: Fluent Bit в CloudWatch Logs, фильтрация и
retention](../../labs/115/README_RU.MD). Кроме неё, состояние логирования легко снять на живом
кластере. Сначала воспроизведите исходную боль и посмотрите, что вообще отдаёт `kubectl logs`:
