...
- **Свободные адреса под мониторингом**: алерт на 20 процентов остатка
  `AvailableIpAddressCount` даёт недели на реакцию вместо аварии в момент релиза.
...
- **ENI** - elastic network interface; число ENI на инстанс и адресов IPv4 на ENI зависит от
  ...
  **warm-пул** - запас таких адресов ради скорости запуска.
...
Лабы у главы пока нет, но всё содержание проверяется на живом кластере. Начните с адресного
