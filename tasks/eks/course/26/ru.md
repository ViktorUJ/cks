...
**Межзональная балансировка.** У NLB cross-zone load balancing по умолчанию **выключен** на
уровне target group (в отличие от ALB, где он всегда включён): NLB в каждой зоне шлёт трафик
только на таргеты своей зоны. Если поды разложены по AZ несимметрично, нагрузка на реплики
получается неравномерной. Включают его тем же `target-group-attributes`:
`cross_zone.load_balancing.enabled=true`. Компромисс - FinOps: выравнивание нагрузки по всем
подам во всех зонах против платы за межзональный трафик (cross-AZ data transfer
тарифицируется). Взаимодействует с `externalTrafficPolicy` (раздел 26.6): `Local` тоже держит
трафик в пределах ноды и усиливает перекос при несимметричном размещении.

**Security groups и дрейф IaC.** С версии v2.6.0 LBC умеет сам создавать frontend security
group для NLB и править правила backend SG на нодах и подах. Если вся сеть и SG ведутся через
Terraform или Terragrunt, эти автоматические изменения дают дрейф состояния: `plan` показывает
правки правил, которых нет в коде. Управляют этим двумя аннотациями:
`aws-load-balancer-manage-backend-security-group-rules: "false"` отдаёт правила backend SG под
контроль вашего IaC, а `aws-load-balancer-security-groups` привязывает к NLB заранее созданные
в Terraform frontend-группы вместо автосоздания. Тогда у SG один владелец, и дрейфа нет.
...
NLB - единственный выбор для приложений на **UDP** (DNS, медиа-стриминг, игровые серверы) и
для **QUIC (HTTP/3)** поверх UDP: ALB работает только с TCP - HTTP, HTTPS и HTTP/2, но не с
UDP и не с QUIC. Если приложению нужен HTTP/3 на входе, его терминируют на NLB (или на своём
прокси за NLB), а не на ALB.

Грубое правило: HTTP-роутинг по путям и хостам - ALB через Ingress (глава 27); чистый L4,
UDP, QUIC, статические IP или максимальная пропускная способность - NLB через Service типа
...
Лаба курса к этой теме: [лаба 108 - AWS Load Balancer Controller: NLB для Service типа
LoadBalancer](../../labs/108/README_RU.MD). Кроме неё, всё проверяется на живом кластере.
