...
Лаба курса к этой теме: [лаба 124 - автомасштабирование приложений: HPA, KEDA,
Prometheus](../../labs/124/README_RU.MD). В ней вы ставите kube-prometheus-stack и KEDA,
описываете `ScaledObject` со scaler `prometheus`, видите своими глазами, что KEDA не заменяет
HPA, а создаёт и ведёт обычный `keda-hpa-*`, затем масштабируете приложение по нагрузке чужих
подов и наблюдаете возврат к минимуму через окно стабилизации; проверка - командой
`check_result`. Запуск - `TASK=124 make run_eks_task`.

Состояние автомасштабирования полезно уметь снимать и на любом рабочем кластере.
...
в `Pending` из-за нехватки нод. Кроме курсовой лабы в репозитории есть отдельная, не
курсовая лаба по автомасштабированию с KEDA и Prometheus [лаборатория 03](../../labs/03/README_RUS.MD): она
