...
```mermaid
flowchart TB
    subgraph k8s["Слой Kubernetes: то, что вы видите в kubectl"]
        direction TB
        s1["Node NotReady"]
        s2["Pod Pending"]
        s3["нет новых нод"]
        s4["PVC Pending"]
        s5["LoadBalancer без адреса"]
    end
    subgraph aws["Слой AWS: то, где лежит причина"]
        direction TB
        a1["IAM-роль ноды,<br>security group,<br>user data"]
        a2["кончились IP<br>в подсети,<br>лимит ENI"]
        a3["квота на vCPU,<br>нет спот-ёмкости"]
        a4["том EBS<br>в другой AZ"]
        a5["теги подсетей,<br>квота на балансировщики"]
    end
    s1 --> a1
    s2 --> a2
    s3 --> a3
    ...
    style a5 fill:#db4437,color:#fff
```
...
flowchart TB
    ...
    reg --> az3["Availability Zone C"]
    az1 --> sub1["подсеть 10.0.0.0/20"]
    az2 --> sub2["подсеть 10.0.16.0/20"]
    az3 --> sub3["подсеть 10.0.32.0/20"]
    sub1 --> n1["ноды и поды"]
    sub2 --> n2["ноды и поды"]
    sub3 --> n3["ноды и поды"]
...
```mermaid
flowchart TB
    root["Management account<br>биллинг, без нагрузок"]
    root --> sec["OU Security"]
    root --> inf["OU Infrastructure"]
    root --> wl["OU Workloads"]
    root --> sb["OU Sandbox"]
    style root fill:#db4437,color:#fff
    style sec fill:#673ab7,color:#fff
    style inf fill:#326ce5,color:#fff
    style wl fill:#0f9d58,color:#fff
    style sb fill:#607d8b,color:#fff
```
...
```mermaid
flowchart LR
    sub["Подсети VPC<br>с тегами"] --> alb["AWS LB Controller<br>ищет kubernetes.io/role/elb"]
    sub --> karp["Karpenter<br>ищет karpenter.sh/discovery"]
    sg["Security group<br>karpenter.sh/discovery"] --> karp
    alb --> lb["ALB или NLB<br>в нужных подсетях"]
