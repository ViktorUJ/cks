...
простое - **для spot набор типов держат максимально широким**.

Вторая часть помощи - **обработка прерываний**. Karpenter ловит события изъятия через очередь
прерываний (SQS плюс EventBridge) и, получив уведомление, заранее поднимает замену, кордонит и
дренирует ноду. Настройка очереди - глава 12: **Karpenter реагирует сам**, если она настроена.
