...
```mermaid
flowchart TB
    subgraph aws["AWS: обслуживает и дежурит"]
        api["kube-apiserver<br>scheduler, CM"]
        etcd["etcd<br>в нескольких AZ"]
    end
    subgraph you["Вы: обслуживаете и дежурите"]
        nodes["ноды и AMI"]
        net["VPC и подсети"]
        access["IAM, RBAC,<br>секреты, аддоны"]
    end
...
**platform version**, которая растёт без вашего участия; даёт SLA на доступность API-сервера;
отдаёт логи control plane в CloudWatch, если вы их включили (глава 2). Взамен вы лишаетесь
...
flowchart TB
    ...
    inc --> aws["Чинит AWS"]
    inc --> you["Чините вы"]
    aws --> a1["CVE в apiserver<br>или etcd"]
    aws --> a2["отказ AZ<br>control plane"]
    you --> y1["CVE в kubelet<br>или containerd"]
    you --> y2["доступ к IMDS<br>из пода"]
    ...
    style y1 fill:#db4437,color:#fff
    style y2 fill:#db4437,color:#fff
```
...
базовые аддоны и их обновления. Он сдвигает границу, но не убирает её и стоит своих
компромиссов; разбираем в главе 9. До тех пор подразумевается кластер, где ноды ваши.
...
namespace вместо кластера на каждую команду (главы 22 и 43). Свобода: control plane закрыт, а
вместе с ним закрыты его настройки.
...
| Extended support | версию можно держать дольше стандартного срока, но дороже: это отсрочка, а не решение (глава 3) |
