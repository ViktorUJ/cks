...
**Savings Plans** и **Reserved Instances** - скидка 30-70% за обязательство тратить
фиксированную сумму (или держать конкретные инстансы) **1 или 3 года**. Compute Savings Plans
гибче: скидка идёт на EC2, Fargate и Lambda независимо от семейства, размера и региона, так что
переезд с `m6i` на `m7g` её не ломает; RI привязаны к типу и зоне и для нод берутся редко.
Обязательство считают **по нижней границе** потребления, пики закрывает спот. **Graviton** - не
модель оплаты, а отдельный источник экономии при сопоставимой производительности.
...
| **Savings Plans** | 30-70% | обязательство 1-3 года | предсказуемая база потребления |
| **Reserved Instances** | 30-70% | привязка к типу и зоне | редкие профили, например GPU |
