...
7. Почему под не достукивается до RDS даже при верном DNS и что такое SG-референс?
8. В чём ловушка NACL с ephemeral ports и почему её не бывает у security group?
9. Назовите причины плавающих DNS-таймаутов: при чём тут `ndots:5`, conntrack и per-ENI лимит.
10. Как NodeLocal DNSCache смягчает DNS-сбои и какую нагрузку он снимает?
11. Почему таргеты в балансировщике `unhealthy` и что показывает `describe-target-health`?
12. Чем 502 отличается от 503 в ответе балансировщика по смыслу для диагностики?
13. Когда в диагностике сетевого обрыва стоит доставать VPC Flow Logs и что там искать?

## Практика

Своей лабы у главы нет: это диагностический runbook. Но все проверки безопасно прогнать на
