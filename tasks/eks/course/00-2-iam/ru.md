...
Условия по тегам дают второй способ раздавать права, и здесь различают две модели. **RBAC в
IAM** это привычный подход: под каждую роль пишут политику с конкретными `Action` и `Resource`.
**ABAC (Attribute-Based Access Control)** вместо перечисления ресурсов сверяет теги: одна
политика с условием `aws:PrincipalTag/Team` открывает доступ к ресурсам с тем же тегом `Team`,
и новой команде не нужна отдельная политика - достаточно проставить тег. В примере выше условие
`Team=platform` это и есть ABAC: право зависит от атрибута принципала, а не от его имени.
...
- **Permissions boundary** - политика-потолок на роли или пользователе, итоговые права это
  ...
  своих сервисов, но не может выдать им больше, чем позволяет boundary. Рабочая норма: для
  всех ролей, которые заводят разработчики и CI/CD-пайплайны, boundary обязателен. Иначе
  пайплайн с правом `iam:CreateRole` фактически может создать роль-администратора и повысить
  себе права; boundary делает такую эскалацию невозможной.
...
Условия работают и в trust policy - это ABAC на приёме роли. Такой документ разрешает принять
роль только принципалам с тегом `Team=platform`, и добавлять их ARN по одному не нужно:
...
- **Внешний доступ под контролем.** IAM Access Analyzer постоянно анализирует resource-based
  политики и trust policy и находит сущности вне аккаунта или Organization, которым открыт
  доступ (external access): чужой аккаунт в trust policy роли, публичный бакет S3, ключ KMS.
  Находки разбирают, а лишний доступ убирают.
...
- **ABAC / RBAC** - доступ по тегам через `aws:PrincipalTag` против доступа по ролям и
  политикам с конкретными действиями и ресурсами.
- **IAM Access Analyzer** - находит внешние доверенные сущности (external access) в
  resource-based политиках и trust policy.
...
4. Чем permissions boundary отличается от обычной политики и от SCP и почему он обязателен
   для ролей от CI/CD?
...
10. Чем ABAC отличается от RBAC в IAM и какое условие лежит в его основе?
11. Зачем нужен IAM Access Analyzer и что он понимает под external access?
