...
# 第 7 章. 位址規劃規模：prefix delegation、secondary CIDR、custom networking
...
## 7.2. Prefix delegation：ENI slot 配置 /28 區塊
...
## 7.4. Secondary CIDR：現有 VPC 中的新位址空間
...
kind: ENIConfig
metadata:
  name: eu-central-1a          # name = 每個 AZ 一個 subnet 時使用 zone name
spec:
  subnet: subnet-0123456789abcdef0   # 同一 AZ 中的 100.64.x subnet
  securityGroups:
...
The course lab for this topic is [實驗 103 - 位址規劃：ENI 限制、prefix delegation、secondary CIDR](../../labs/103/README_TW.MD). Beyond it, verify the content on a live cluster. Start with the CNI operating mode:
