...
вы делаете namespace-restore в живой кластер, видите non-destructive поведение (существующие
объекты не перезаписываются) и разбираете, почему откат версии кластера не возвращает удалённый
namespace; проверка - командой `check_result`. Запуск - `TASK=122 make run_eks_task`.

Помимо лабы, состояние восстановления видно из инструментов. Начните с
