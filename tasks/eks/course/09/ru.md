...
Различие удобно держать как шкалу ответственности: слева self-managed, где на вас всё, справа
Fargate и Auto Mode, где ноды почти целиком у AWS, а managed node groups посередине.

```mermaid
flowchart TB
    q["Под нужно<br>где-то запустить"] --> self["Self-managed:<br>всё на вас"]
    q --> mng["Managed node<br>groups"]
    q --> auto["EKS Auto Mode:<br>ноды у AWS"]
    q --> far["Fargate:<br>под = микро-VM"]
    self --> more["Больше контроля,<br>больше труда"]
    far --> less["Меньше труда,<br>меньше контроля"]
    auto --> less
...
приватных подсетях. Уместен для изолированных нагрузок, пачек job'ов и сервисов, где не нужен
...
```mermaid
flowchart TB
    aws["AWS в Auto Mode"] --> nodes["Ноды-appliance,<br>Bottlerocket"]
    aws --> scale["Karpenter и<br>spot-события"]
    aws --> addons["IP, DNS, NP,<br>EBS CSI, ELB"]
    you["Остаётся на вас"] --> cont["Контейнеры,<br>requests и limits"]
    you --> vpc["VPC и конфиг<br>кластера"]
    you --> vols["Тома из PVC,<br>балансировщики"]
...
| Тонкий контроль Karpenter | дефолтный NodePool не править | своя политика consolidation и budgets (глава 12) |
...
тонкий контроль над Karpenter, стоимость критична настолько, что наценка за управление не
