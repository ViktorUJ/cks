...
середину файла - валятся. Причём errno у них РАЗНЫЕ, и это первое, что стоит заметить:
`rename` отдаёт `ENOSYS` (`Function not implemented`) - вызова в драйвере нет вовсе, а
`append` и запись в середину отдают `EPERM` (`Operation not permitted`) - операция есть, но
запрещена. Разница пригодится в 25.7: `ENOSYS` настройками не лечится, `EPERM` иногда лечится
опциями монтирования. Это не баг драйвера и не вопрос прав POSIX. Причина глубже: S3 -
объектное хранилище, а не файловая система. Mountpoint даёт
...
| `cache` | `volumeAttributes` | тип локального кэша: `emptyDir` или `ephemeral` |
| `metadata-ttl ...` | `mountOptions` | TTL кэша метаданных (секунды/`indefinite`) |
| `storageClassName: ""` | PV и PVC | обязателен для статики |

**Кэш повторного чтения.** Mountpoint умеет кэшировать данные и метаданные объектов, чтобы
повторные чтения одного файла не ходили в S3 заново - это ускоряет read-heavy нагрузки. В
CSI-драйвере v2 локальный кэш данных задаётся не флагом, а атрибутами тома: `cache: emptyDir`
кладёт кэш на локальный том ноды, а `cacheEmptyDirSizeLimit` ограничивает его размер (задавать
обязательно, иначе кэш съест диск ноды). `cacheEmptyDirMedium: Memory` переносит кэш в tmpfs
(RAM) ради меньших задержек ценой памяти ноды. Кэш метаданных включается отдельно - опцией
`metadata-ttl` в `mountOptions`. Для кэша на выделенном томе (EBS или instance store) есть тип
`cache: ephemeral` с `cacheEphemeralStorageClassName` и `cacheEphemeralStorageResourceRequest`.

```yaml
    volumeAttributes:
      bucketName: amzn-s3-demo-bucket
      cache: emptyDir              # локальный кэш данных на ноде
      cacheEmptyDirSizeLimit: 2Gi  # лимит обязателен, иначе кэш займёт весь диск
```

В v1 кэш задавался путём через `cache` в `mountOptions`; в v2 это устарело - путь игнорируется,
драйвер создаёт `emptyDir`-том сам. Указывайте кэш только атрибутами тома.
...
### S3 Express One Zone (directory buckets) с Mountpoint

Особый случай - directory buckets класса хранения **S3 Express One Zone**. Это зональное
хранилище: данные лежат в одной зоне доступности, рядом с compute (можно колокировать с нодами
EKS в той же AZ), что даёт самую низкую задержку и высокий IOPS - сотни тысяч запросов в
секунду на бакет. Плата за это двойная. Первое - зональность: одна AZ работает на задержку, а
не на durability между зонами, при отказе зоны данные недоступны. Второе - выше стоимость
хранения за гигабайт, чем у general purpose. Есть и планировочное следствие: том привязан к
зоне бакета, поэтому под с ним держат в той же AZ, иначе теряется смысл колокации и растёт
задержка. Это не замена general purpose S3 для надёжного долговременного хранения.

Для Mountpoint у directory buckets есть важное послабление: они поддерживают `append` к
существующему объекту, которого нет у обычных general purpose бакетов (25.5). Дозапись в конец
файла работает, то есть часть POSIX-ограничений снимается. Остальные запреты из 25.5 (нет
`rename`, нет записи в середину, нет symlink) остаются - объектная природа никуда не девается.

Когда брать directory bucket: критичны низкая задержка и высокий IOPS, а данные переживут
потерю зоны, потому что лежат ещё где-то (исходный датасет в general purpose S3, возможность
перегенерировать) - обучение ML, интерактивная аналитика, обработка медиа. Когда general
purpose: нужна durability между зонами, долговременное хранение единственной копии, доступ из
нескольких AZ или запись без привязки пода к одной зоне. Directory bucket - это ускоритель
горячих данных, а не место для единственной копии.
...
| `Function not implemented` на `rename` | вызова нет в драйвере (25.5) | паттерн записи приложения |
| `Operation not permitted` на `append`, перезаписи, удалении | ограничения Mountpoint и mount options (25.5) | паттерн записи, `allow-overwrite`, `allow-delete` |
...
несовместимость приложения с объектной моделью (25.5). Смотрите на errno: `ENOSYS` у `rename`
означает «этого в драйвере нет и не появится», а `EPERM` у перезаписи и удаления снимается
опциями `allow-overwrite` и `allow-delete`, если это осознанное решение. Лечится либо
переходом на SDK (25.3),
...
S3 API виснут по таймауту. Заодно gateway endpoint уводит трафик к S3 с NAT Gateway, так что
чтение датасетов не тарифицируется как NAT-трафик. Endpoints и приватный трафик - глава 31.
...
- **Приватный доступ через gateway endpoint.** В приватных подсетях трафик к S3 идёт через
  gateway endpoint, а не через NAT Gateway: чтение не идёт наружу и не тарифицируется как
  NAT-трафик (глава 31).
- **Локальный кэш для повторного чтения.** Для read-heavy датасетов включают `cache: emptyDir`
  с `cacheEmptyDirSizeLimit`: повторные чтения бьют в кэш ноды, а не в S3. Метаданные кэширует
  `metadata-ttl`.
...
- **префикс** - часть ключа до `/`, из которой Mountpoint эмулирует каталог; настоящих
  каталогов в S3 нет.
- **локальный кэш** - кэш данных Mountpoint на томе ноды (`cache: emptyDir`/`ephemeral`),
  ускоряющий повторное чтение; кэш метаданных задаётся `metadata-ttl`.
- **gateway endpoint** - VPC-endpoint типа Gateway для приватного доступа к S3 без интернета
  (глава 31).
- **S3 Express One Zone** - зональный класс хранения (directory buckets) с низкой задержкой и
  высоким IOPS в одной AZ; в отличие от general purpose бакетов поддерживает `append`.
...
- Повторное чтение ускоряют локальным кэшем (`cache: emptyDir` с `cacheEmptyDirSizeLimit`,
  `metadata-ttl`), а трафик к S3 из приватной подсети уводят на gateway endpoint мимо NAT
  Gateway (глава 31).
...
группа - приложение падает на `rename` (`Function not implemented`), на `append` или записи в
середину файла (`Operation not permitted`). Это не чинится настройкой: приложение ждёт от S3
поведения POSIX-ФС, которого у объектного хранилища нет. Правильный ответ - либо перевести
код на AWS SDK (и тогда CSI вообще
...
13. Зачем в приватной подсети нужен gateway endpoint для S3 и как он экономит на NAT Gateway?
14. Как включить локальный кэш данных Mountpoint и зачем задавать `cacheEmptyDirSizeLimit`?
15. Что даёт S3 Express One Zone для Mountpoint и в чём плата за зональность?

## Практика

Лаба курса к этой теме: [лаба 129 - Mountpoint for S3: где ломается семантика файлов и почему
нет бэкапа](../../labs/129/README_RU.MD). Там статический PV на реальный бакет, успешные
операции (новый объект и чтение) и три отказа подряд с разбором errno, а в финале - почему у
такого PVC нет снапшота и что защищает данные вместо него. Результат проверяется командой
`check_result`.

Ниже - то же самое на любом своём кластере. Сначала посмотрите на бакет
...
`ReadWriteMany`. Берите образ с shell и утилитами (`busybox`), иначе `kubectl exec` не с чем
будет запускать. В поде проверьте, что чтение и создание нового файла работают
(`kubectl exec ... -- cat /data/<key>` и запись нового ключа), а затем убедитесь, что
`mv /data/a /data/b` падает с `Function not implemented`, а дозапись `echo x >> /data/existing`
и запись в середину через `dd ... seek=...` - с `Operation not permitted`. Заодно попробуйте
перезаписать и удалить файл: тоже `Operation not permitted`, пока не включены `allow-overwrite`
и `allow-delete`. Сравните с `ReadOnlyMany`: смонтируйте тот же
