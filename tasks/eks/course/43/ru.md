...
**Коммит и Spot конкурируют за одну базу.** Savings Plans не применяются к Spot-потреблению:
Spot коммитом не покрывается и дополнительной скидки поверх spot-цены не получает. Отсюда
типовая ошибка: коммит закупают по текущему потреблению, а потом часть парка переводят на Spot
(Karpenter или node group) - покрываемая база уменьшается, коммит остаётся недоиспользованным.
«Потом выровняется» не работает: коммит часовой, неиспользованный остаток часа на следующий не
переносится, недобор сгорает каждый час, а не подбивается в конце срока. Поэтому из baseline
вычитают ту часть, которую планируют держать на Spot, и коммитят непрерываемый остаток. Но
«вычитать Spot» - это не «вычитать всю мощность spot-пулов»: fallback на On-Demand при нехватке
spot-ёмкости (глава 13) возвращает часть потребления под коммит, поэтому вычитают устойчиво
достигаемую долю Spot, а не проектную, и пересматривают коммит по факту, а не по плану. Порядок
применения: Savings Plans идут после Reserved Instances, EC2 Instance Savings Plans раньше
Compute Savings Plans, а внутри - начиная с потребления с наибольшим процентом скидки; это
объясняет, почему на смешанном парке коммит уходит не туда, куда ждали.
...
- **скидочные модели покрывают не всё:** под Compute Savings Plans попадают EC2, Fargate,
  Lambda и SageMaker AI, а почасовая плата за control plane EKS в этот список не входит, и
  постоянная строка за кластер скидочными моделями не уменьшается (глава 9).
...
- **Мониторинг коммита.** В Cost Explorer есть отчёт Savings Plans utilization (сколько коммита
  реально израсходовано) и отчёт Savings Plans coverage (какая доля подходящего потребления
  покрыта коммитом), а в AWS Budgets - отдельный тип бюджета для Savings Plans, по utilization
  и по coverage, с алертами через SNS. За utilization следят так же, как за перерасходом:
  падение после перевода нагрузок на Spot видно сразу, а не через месяц в счёте.
...
- **Compute Savings Plans** - обязательство по расходу в час на 1-3 года в обмен на скидку,
  гибкое по семействам инстансов, региону и Fargate/Lambda; коммит часовой, между часами не
  переносится и на Spot не распространяется, а его расход видно в отчётах Savings Plans
  utilization (израсходовано) и coverage (покрыто) в Cost Explorer.
...
- Spot и коммит конкурируют за одну базу: Savings Plans не покрывают Spot, а часовой коммит не
  переносится между часами, поэтому из baseline вычитают устойчиво достигаемую долю Spot.
...
11. Почему покупка Savings Plans и перевод парка на Spot конфликтуют и что вычитают из
    baseline перед тем, как коммитить?
12. Что специфично для счёта EKS: control plane, extended support, Fargate?
13. Какие статьи трафика и хранения оптимизируют и какие главы за них отвечают?
14. Каков приоритет оптимизации и почему нельзя коммитить Savings Plans до right-sizing?
15. Зачем подключать OpenCost/Kubecost к Cost and Usage Report и чем Cost Anomaly Detection
    дополняет AWS Budgets?

## Практика

Стоимость трафика также разбирается в [лабе 117 - Трафик и стоимость: NAT по зонам против
одного NAT, VPC endpoints, cross-AZ](../../labs/117/README_RU.MD). Своей отдельной лабы у
этой главы нет, но всю картину видно на живом кластере и в консоли AWS. Начните с
