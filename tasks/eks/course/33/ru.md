...
```mermaid
flowchart TB
    cluster["EKS: ноды, поды, control plane"]
...
Своей лабы у главы пока нет, но текущее состояние метрик легко проверить на живом кластере.
Сначала посмотрите, есть ли базовый слой и отвечает ли Metrics API:
