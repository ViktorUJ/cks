...
снимка - обычно это дамп средствами самой СУБД либо заморозка файловой системы (fs-freeze)
перед снимком и снятие заморозки после.

И здесь ограничение, которое легко принять за решённую задачу: **у AWS Backup нет хуков внутри
подов**. Сервис снимает тома как есть и выполнить команду в контейнере до и после снимка не
умеет: VSS-механизм согласования у него есть только для EC2 с Windows, exec-хуков в поды нет
вовсе. Отсюда три рабочих пути для StatefulSet с СУБД: держать нативные дампы базы в S3 рядом с
бэкапом AWS Backup; строить внешнюю обвязку (у Amazon Data Lifecycle Manager есть
pre/post-скрипты через SSM для снапшотов EBS, но это уровень инстанса, а не пода); либо взять
Velero, у которого
хуки бэкапа штатные - аннотации `pre.hook.backup.velero.io/command` и
`post.hook.backup.velero.io/command` выполняют команду в контейнере до и после снятия бэкапа
(глава 42). Практика чаще всего сводится к первому варианту: нативные дампы для данных БД,
AWS Backup - для состояния кластера и томов.
...
который складывает бэкап объектов в S3-бакет, умеет бэкапить по namespace или label, снимает
снапшоты томов через CSI и, в отличие от AWS Backup, запускает хуки в подах до и после снятия
бэкапа - именно этим закрывают согласованность СУБД. Он живёт внутри кластера и ближе к
Kubernetes, тогда как AWS Backup -
...
- **Не полагаются на AWS Backup для баз данных без обвязки.** Снапшот тома всегда
  crash-consistent, а хуков внутри подов у сервиса нет: для СУБД настраивают нативные дампы,
  внешнюю автоматизацию или Velero с хуками бэкапа (глава 42).
...
- **crash-consistent / application-consistent** - снимок без остановки записи против снимка с
  согласованием на уровне приложения. У AWS Backup для EKS доступен только первый: хуков в
  подах нет, второй обеспечивают дампами БД, внешней обвязкой или хуками Velero.
...
- Снапшоты томов crash-consistent, а хуков внутри подов у AWS Backup нет: согласованность БД
  на уровне приложения дают нативные дампы, внешняя обвязка или Velero с хуками (глава 42).
...
14. Почему application-consistent бэкап СУБД нельзя получить одним AWS Backup и какие есть
    варианты закрыть это?

## Практика

Лаба курса к этой теме: [лаба 122 - AWS Backup для EKS](../../labs/122/README_RU.MD). В ней
вы включаете opt-in, снимаете on-demand бэкап кластера с томом на gp3, разбираете composite
recovery point (parent и вложенные точки EKS и EBS) и делаете namespace-restore; проверка -
командой `check_result`. Запуск - `TASK=122 make run_eks_task`.

Бэкап тома EBS также разбирается в [лабе 129 - Mountpoint for S3: где ломается семантика
файлов и почему нет бэкапа](../../labs/129/README_RU.MD) - там показано, почему у тома на S3
нет снапшота и что защищает данные вместо него, в отличие от EBS-тома этой главы.

Помимо лабы, состояние бэкапа видно из AWS CLI. Сначала проверьте opt-in
