...
| `--source=gateway-httproute`, `--source=gateway-grpcroute` | имена из ресурсов Gateway API (глава 28) |
...
На Gateway API это переносится без переучивания, но с двумя оговорками. Первая: контроллеру
нужны права в кластере на ресурсы `gateway.networking.k8s.io` (`gateways`, `httproutes`,
`grpcroutes`), иначе он просто не увидит маршруты. Вторая - распределение аннотаций, на котором
спотыкаются: имя берётся из `spec.hostnames` маршрута, аннотацию
`external-dns.alpha.kubernetes.io/target` external-dns читает **только с `Gateway`**, а
остальные аннотации (`hostname`, `ttl`, провайдерские) - **только с маршрута**. Поставленные
наоборот, они молча игнорируются.
У `TCPRoute` и `UDPRoute` имён в спеке нет вовсе, поэтому им hostname задают аннотацией.
...
Отдельная тема - API Route 53, у которого есть лимиты запросов. Как часто external-dns
синхронизирует зону, задаёт `--interval` (по умолчанию `1m`); слишком короткий интервал
на большой зоне быстрее упирается в троттлинг. Чтобы не занижать `--interval` ради
отзывчивости, включают `--events` - тогда цикл дополнительно запускается по изменениям
объектов, а не только по таймеру. Массовые изменения укрупняют флагами
`--aws-batch-change-size` (сколько изменений в одном батче, по умолчанию `1000`) и
`--aws-batch-change-interval` (пауза между батчами), чтобы реже дёргать API.
...
## 29.8. Грабли, на которые наступают
...
- **Троттлинг Route 53 API.** На больших зонах частые синхронизации упираются в лимиты
  запросов. Держат `--interval` умеренным, включают `--events` для отзывчивости и
  укрупняют изменения через `--aws-batch-change-size` и `--aws-batch-change-interval`.
...
Лаба курса к этой теме: [лаба 109 - Ingress через ALB с сертификатом ACM, external-dns и Route
53](../../labs/109/README_RU.MD). Кроме неё, всё проверяется на живом кластере. Сначала
посмотрите, стоит ли external-dns и здоров ли он, и загляните в его флаги:
