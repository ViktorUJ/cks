...
Модель для многих кластеров - **hub и spoke**. Один hub-кластер несёт Argo CD (или Flux) и
управляет многими spoke-кластерами: агент на hub применяет манифесты в каждый целевой кластер.
Парк кластеров EKS (глава 32) перестаёт требовать по инсталляции агента на кластер.
...
- **ApplicationSet** - контроллер Argo CD, генерирующий `Application` по шаблону; cluster
  generator создаёт по одному на каждый подключённый кластер.
...
12. Где проходит граница между IaC (Terraform) и GitOps и почему её нельзя размывать?
13. Как GitOps-агент на EKS получает доступ к ECR и почему секреты не коммитят в Git?
