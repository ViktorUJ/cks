...
dependency "eks_addon_s3_csi_irsa" {
  config_path = "../eks_addon_s3_csi_irsa"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/129/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/129/worker/files/worker.sh"
    ssh = {
