...
dependency "eks_addon_ebs_irsa" {
  config_path = "../eks_addon_ebs_irsa"
}

dependency "eks_backup_iam" {
  config_path = "../eks_backup_iam"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/122/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/122/worker/files/worker.sh"
    ssh = {
