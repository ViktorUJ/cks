...
dependency "eks_karpenter_hardened_vng" {
  config_path = "../eks_karpenter_hardened_vng"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/116/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/116/worker/files/worker.sh"
    ssh = {
