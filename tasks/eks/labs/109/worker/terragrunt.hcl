...
dependency "eks_extdns_irsa" {
  config_path = "../eks_extdns_irsa"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/109/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/109/worker/files/worker.sh"
    ssh = {
