...
| Acceptance criteria    | - `disallow-latest-tag-binding` has `validationActions: ["Deny"]`;<br/>- Pod `latest-deny-pod` in `eks-127` does NOT exist;<br/>- file `denied.txt` is not empty and contains the words `latest` and `запрещён`. |
---
