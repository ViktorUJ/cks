...
dependency "eks_irsa_break_demo" {
  config_path = "../eks_irsa_break_demo"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/121/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/121/worker/files/worker.sh"
    ssh = {
