...
inputs = {
  ...
  work_pc = {
    ...
    # Апгрейд и откат control plane (задания 3 и 6) занимают по 20-40 минут
    # каждый, поэтому времени окружения дано больше, чем в базовой лабе 101.
    exam_time_minutes = "900"
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/113/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/113/worker/files/worker.sh"
    ssh = {
