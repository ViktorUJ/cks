...
dependency "fluentbit_irsa" {
  config_path = "../fluentbit_irsa"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/115/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/115/worker/files/worker.sh"
    ssh = {
