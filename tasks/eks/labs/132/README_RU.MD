...
| :-----------------: | :---------------------------------------------------------- |
| Что делаем          | Установите Cilium через Helm в режиме chaining (`cni.chainingMode: aws-cni`), ограничив DaemonSet `cilium` и `cilium-operator` через `nodeSelector`/`tolerations` только на ноды с меткой `work_type: cilium-demo` (нода из задания 2 уже существует, поэтому DaemonSet сразу на неё встанет). Дождитесь, пока все поды Cilium перейдут в `Running` и находятся только на нодах `cilium-demo`. Поды `api` и `client` были запущены до установки Cilium и не подключены к цепочке - выполните `kubectl rollout restart` для обоих Deployment, чтобы они переподключились через Cilium (это явно описано в документации Cilium для chaining). Установите Cilium CLI и подтвердите `cilium status` - строки `Cilium:` и `OK`. Сохраните вывод в файл `/var/work/tests/artifacts/3/cilium_status.txt`. |
...
| :-----------------: | :---------------------------------------------------------- |
| Что делаем          | Создайте `CiliumNetworkPolicy` `cilium-fqdn-allow-aws` в `eks-132`: `endpointSelector` на `app: client`, egress разрешает только `toFQDNs.matchName: aws.amazon.com` (порт `443/TCP`) плюс DNS к `kube-dns` в `kube-system` на `53`. Проверьте, что `client` резолвит `aws.amazon.com` и `curl` до него проходит - сохраните `HTTP_CODE=<код>` в файл `/var/work/tests/artifacts/6/allowed_fqdn.txt`. Проверьте `curl` до другого внешнего домена (например `example.com`) с таймаутом - сохраните результат (не успешный код) в файл `/var/work/tests/artifacts/6/blocked_other_domain.txt`. Правил по DNS-имени у встроенного VPC CNI NetworkPolicy нет (глава 8). |
