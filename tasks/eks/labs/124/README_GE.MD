...
| :-----------------: | :---------------------------------------------------------- |
| რას ვაკეთებთ          | გაუშვით `kubectl get hpa -A` და შეადარეთ output დავალება 5-7-ში დანახულს. შეინახეთ ფაილში `/var/work/tests/artifacts/8/hpa_compare.txt` 2-3 წინადადება: რითი განსხვავდება ამ output-ში ჩვეულებრივი HPA HPA `keda-hpa-demo-app`-ისგან, და განმარტება იმისა, რომ KEDA არ ცვლის HPA-ს - ის მას მართავს, ანუ `управляет` მას (ახსენეთ სიტყვები `HPA`, `KEDA`, `keda-hpa` და ფუძე `управля`). |
| მიღების კრიტერიუმები    | - ფაილი `8/hpa_compare.txt` არ არის ცარიელი;<br/>- შეიცავს სიტყვებს `HPA`, `KEDA`, `keda-hpa` და ფუძეს "управля". |
---
