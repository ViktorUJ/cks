...
dependency "eks_amp_workspace" {
  config_path = "../eks_amp_workspace"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/114/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/114/worker/files/worker.sh"
    ssh = {
