...
# В этой лабе нет отдельного компонента Karpenter: EKS Auto Mode (глава 9) поднимает
# ноды и масштабирование сам, без внешнего контроллера Karpenter и без отдельной IAM-
# роли нод, которую в других лабах создаёт eks_v2_karpenter. Параметр
# karpenter_node_iam_role_name у модуля eks_v2_work_pc не обязателен (default ""),
# поэтому просто не передаём его.
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/125/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/125/worker/files/worker.sh"
    ssh = {
