...
# только для порядка применения: роль LBC должна существовать до того, как
# студент по SSH ставит Helm-чарт aws-load-balancer-controller.
dependency "eks_lbc_irsa" {
  config_path = "../eks_lbc_irsa"
}

dependency "eks_network_break_demo" {
  config_path = "../eks_network_break_demo"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/120/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/120/worker/files/worker.sh"
    ssh = {
