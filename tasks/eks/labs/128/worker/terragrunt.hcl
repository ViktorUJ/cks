...
dependency "eks_vpclattice_irsa" {
  config_path = "../eks_vpclattice_irsa"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/128/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/128/worker/files/worker.sh"
    ssh = {
