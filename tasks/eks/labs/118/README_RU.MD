...
| :-----------------: | :---------------------------------------------------------- |
| Что делаем          | Важная деталь: при включённом `selfHeal` контроллер откатывает дрейф за секунды, и застать `OutOfSync` глазами практически невозможно. Поэтому дрейф показывают в два шага. Сначала выключите только самолечение: `selfHeal: false` (`prune` оставьте `true`). Затем поменяйте живое состояние в обход Git - `kubectl scale deployment demo-app -n eks-118 --replicas=5` - и убедитесь, что Application перешёл в `OutOfSync` и там остался (без self-heal дрейф не лечится). Снимите этот момент. Потом верните `selfHeal: true` и дождитесь, пока self-heal сам откатит реплики к `2`, а Application снова станет `Synced`. Снимите второй момент. Оба снимка (`sync`/`health` и итоговое `spec.replicas`) сохраните в файл `/var/work/tests/artifacts/4/drift.txt`. |
...
---
|        **6**        | **Что prune удаляет, а что нет: владение ресурсом**         |
| :-----------------: | :---------------------------------------------------------- |
| Что делаем          | Создайте в `eks-118` руками ConfigMap `manual-extra`, которого нет в `gitops-demo/`: `kubectl create configmap manual-extra -n eks-118 --from-literal=note=manual`. Форсируйте сверку (`kubectl patch application demo-app -n argocd --type=merge -p '{"metadata":{"annotations":{"argocd.argoproj.io/refresh":"hard"}}}'`) и убедитесь в неожиданном: `prune=true` включён, но `manual-extra` **не удаляется**, а Application остаётся `Synced`. Причина: Argo CD владеет только теми объектами, которые помечены как принадлежащие приложению (в версии 3.x - аннотация `argocd.argoproj.io/tracking-id`, посмотрите её у `demo-config`); созданный руками объект для него посторонний (orphaned) и в область prune не попадает. Теперь пометьте его как принадлежащий приложению: `kubectl annotate configmap manual-extra -n eks-118 'argocd.argoproj.io/tracking-id=demo-app:/ConfigMap:eks-118/manual-extra' --overwrite`, снова форсируйте сверку - теперь объект отслеживается, в Git его нет, и prune его удаляет. В файл `/var/work/tests/artifacts/6/prune.txt` запишите оба наблюдения (не удалён без tracking, удалён с tracking) и объясните словами, что prune убирает только отслеживаемые ресурсы, отсутствующие в Git, поэтому `demo-app` и `demo-config` остаются на месте. |
...
| :-----------------: | :---------------------------------------------------------- |
| Что делаем          | Временно отключите `syncPolicy.automated` у `Application` (иначе self-heal сразу откатит следующий шаг). Добавьте Deployment `demo-app` заведомо неработающий `readinessProbe`. Внимание: учебный образ `ping_pong` отвечает `200` на любой путь, поэтому проба на «несуществующий URL» пройдёт успешно - направьте пробу на **закрытый порт**, например `httpGet` на порт `9999`. Тогда новые поды не проходят readiness. Убедитесь, что `.status.health.status` стал `Progressing` (а после истечения `progressDeadlineSeconds` Deployment станет `Degraded`), при этом `.status.sync.status` остаётся `Synced`. Обратите внимание почему: `replicas` описаны в Git, поэтому их ручная правка давала `OutOfSync` (задание 4), а `readinessProbe` в Git-манифесте нет, и добавленное поле в расхождение не попадает. Объясните в файле `/var/work/tests/artifacts/7/health.txt`, почему `Synced` и `Degraded`/`Progressing` возможны одновременно: sync status сравнивает живое состояние с Git, health status - это реальная готовность ресурса, это две независимые оси. В конце верните `syncPolicy.automated` (`selfHeal`, `prune`) обратно и уберите пробу **вручную** (`kubectl patch ... --type=json` с операцией `remove`): проверьте сами, что self-heal эту поломку не лечит, ведь для Argo CD расхождения с Git нет (`Synced`) - поля, которых нет в манифесте, остаются на ответственности оператора. |
...
> **Сначала выключите `selfHeal`, потом снимайте объекты, иначе Argo CD восстановит их
> сами.** Пока `Application` `demo-app` синхронизирован с `selfHeal: true`, удаление
> `Deployment demo-app` или `ConfigMap demo-config` вручную (или через удаление namespace
> `eks-118`) контроллер откатит за секунды - именно это поведение вы отрабатывали в задании
> 4. AWS-ресурсов лаба не создаёт, риска с осиротевшим ENI здесь нет, но Argo CD ставился
> вручную через Helm, и `TASK=118 make delete_eks_task` про него не знает.

```bash
# 1. Выключить autosync у Application, иначе снятие объектов ниже откатится само
kubectl patch application demo-app -n argocd --type=merge \
  -p '{"spec":{"syncPolicy":{"automated":null}}}'

# 2. Снять Application (после этого Argo CD больше не следит за demo-app и demo-config)
kubectl delete application demo-app -n argocd --ignore-not-found

# 3. Снять сам Argo CD и обе нагрузки лабы
helm uninstall argocd -n argocd
kubectl delete namespace argocd eks-118 --ignore-not-found

# 4. Дождаться, пока Karpenter уберёт NodeClaim и EC2-ноду (останутся только fargate-*)
...
Если `destroy` встал на security group нод, значит остался осиротевший ENI. Найдите и
удалите его:
