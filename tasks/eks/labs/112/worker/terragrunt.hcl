...
dependency "eks_fargate_workload" {
  config_path = "../eks_fargate_workload"
}
...
inputs = {
  ...
  work_pc = {
    ...
    test_url          = "${local.vars.locals.raw_base_url}/tasks/eks/labs/112/worker/files/tests.bats"
    task_script_url   = "${local.vars.locals.raw_base_url}/tasks/eks/labs/112/worker/files/worker.sh"
    ssh = {
