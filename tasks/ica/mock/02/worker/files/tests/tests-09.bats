...
# Task 20: Selective Sidecar Injection (2 points)

@test "20.1 Namespace turquoise has istio-injection=enabled label" {
...
@test "20.2 turquoise-echo deployment exists" {
...
@test "20.3 turquoise-echo deployment has sidecar.istio.io/inject=false annotation" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "20.4 turquoise-echo pod has only 1 container (no sidecar)" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "20.5 sleep-turquoise pod exists" {
  echo '0.25' >> /var/work/tests/result/all
...
@test "20.6 sleep-turquoise pod has 2 containers (app + istio-proxy)" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "20.7 sleep-turquoise has istio-proxy container" {
  echo '0.0' >> /var/work/tests/result/all
  ...
  if [[ "$proxy_exists" -ge "1" ]]; then
    echo '0.0' >> /var/work/tests/result/ok
    result=0
  fi
  [ "$result" == "0" ]
}

# Total: 2 points for Task 20
