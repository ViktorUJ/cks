...
@test "6.1 VirtualService exists in gold namespace" {
...
@test "6.2 DestinationRule exists with v1 and v2 subsets" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "6.3 VirtualService has two route destinations" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "6.4 v1 subset has 50% weight" {
  echo '0.5' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "6.5 v2 subset has 50% weight" {
  echo '0.5' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "6.6 Service is accessible and returns 200" {
  echo '0.5' >> /var/work/tests/result/all
