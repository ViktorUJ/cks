...
@test "7.1 Gateway exists in istio-system namespace" {
...
@test "7.2 Gateway selector is istio: ingressgateway" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "7.3 Gateway hosts includes echo.example.com" {
  echo '0.5' >> /var/work/tests/result/all
  gw_name=$(kubectl get gateway -n istio-system --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "7.4 VirtualService exists in copper namespace" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "7.5 VirtualService hosts includes echo.example.com" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "7.6 VirtualService references the Gateway" {
  echo '0.5' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "7.7 VirtualService routes to copper-echo service" {
  echo '0.5' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "7.8 Service is accessible via ingress gateway" {
  echo '0.5' >> /var/work/tests/result/all
