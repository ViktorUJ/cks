...
# Task 23: Gateway routing for /moon path (2 points)

@test "23.1 Gateway demo-gateway exists in istio-system" {
  echo '0.4' >> /var/work/tests/result/all
  kubectl get gateway demo-gateway -n istio-system --context $CONTEXT > /dev/null 2>&1
  result=$?
  if [[ "$result" == "0" ]]; then
    echo '0.4' >> /var/work/tests/result/ok
  fi
  [ "$result" == "0" ]
}

@test "23.2 VirtualService exists in sapphire namespace" {
  echo '0.4' >> /var/work/tests/result/all
...
@test "23.3 VirtualService routes to echo service" {
  echo '0.4' >> /var/work/tests/result/all
...
@test "23.4 VirtualService has /moon path configured" {
  echo '0.4' >> /var/work/tests/result/all
...
@test "23.5 VirtualService references demo-gateway" {
  echo '0.4' >> /var/work/tests/result/all
