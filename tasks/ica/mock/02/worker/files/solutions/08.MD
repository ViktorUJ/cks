...
### Step 1.5: Ensure Sidecars are Injected

Since the namespace and pods were created together, restart pods to ensure the sidecar injector works.

```bash
kubectl rollout restart deployment -n magenta 2>/dev/null || kubectl delete pod --all -n magenta
kubectl wait --for=condition=ready pod -l app=magenta-echo -n magenta --timeout=60s
```
...
```bash
# Get NodePort for istio-ingressgateway
