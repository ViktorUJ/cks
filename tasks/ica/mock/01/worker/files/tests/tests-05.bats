...
@test "5.1 VirtualService exists in gray namespace" {
...
@test "5.2 Retry attempts configured to 2" {
...
@test "5.3 Per-try timeout configured to 1s" {
  echo '1' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "5.4 Retry configured for 5xx errors" {
  echo '1' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "5.5 Service responds to /blackhole endpoint (triggers retries)" {
  echo '1' >> /var/work/tests/result/all
