...
# Task 13: Configure Traffic Mirroring (3 points)

@test "13.1 VirtualService exists in platinum namespace" {
  echo '0.84' >> /var/work/tests/result/all
...
@test "13.2 DestinationRule exists with v1 and v2 subsets" {
  echo '0.84' >> /var/work/tests/result/all
...
@test "13.3 Primary route goes to v1 subset with 100% weight" {
  echo '0.84' >> /var/work/tests/result/all
...
@test "13.4 Mirror configuration exists" {
  echo '0.84' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "13.5 Mirror target is v2 subset" {
  echo '0.84' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "13.6 Mirror percentage is 50%" {
  echo '0.47' >> /var/work/tests/result/all
  vs_name=$(kubectl get virtualservice -n $NAMESPACE --context $CONTEXT -o jsonpath='{.items[0].metadata.name}')
...
@test "13.7 v1 pod receives requests" {
  echo '0.47' >> /var/work/tests/result/all
...
@test "13.8 Mirror ratio approximately 50% (v2 gets ~50% of v1 traffic)" {
  echo '0.84' >> /var/work/tests/result/all
