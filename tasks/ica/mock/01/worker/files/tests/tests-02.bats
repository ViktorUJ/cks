...
@test "2.1 Orange namespace does NOT have sidecar injection label" {
...
@test "2.2 Pod sidecar-pod exists in orange namespace" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "2.3 Pod sidecar-pod has istio-proxy container" {
...
@test "2.4 Pod sidecar-pod has exactly 2 containers" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "2.5 Pod no-sidecar-pod exists" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "2.6 Pod no-sidecar-pod does NOT have istio-proxy container" {
  echo '1' >> /var/work/tests/result/all
...
@test "2.7 Pod no-sidecar-pod has exactly 1 container" {
  echo '0.5' >> /var/work/tests/result/all
