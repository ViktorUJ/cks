...
# Task 06: Configure AuthorizationPolicy for Specific Paths (3 points)

@test "6.1 AuthorizationPolicy exists in blue namespace" {
  echo '1.8' >> /var/work/tests/result/all
...
@test "6.2 AuthorizationPolicy name is 'sun-and-moon'" {
  echo '0.9' >> /var/work/tests/result/all
...
@test "6.3 AuthorizationPolicy action is ALLOW" {
  echo '0.9' >> /var/work/tests/result/all
...
@test "6.4 AuthorizationPolicy paths include /sun and /moon" {
  echo '0.9' >> /var/work/tests/result/all
...
@test "6.5 Access to /sun is allowed (200)" {
...
@test "6.6 Access to /moon is allowed (200)" {
  echo '0.5' >> /var/work/tests/result/all
...
@test "6.7 Access to /pluto is denied (403)" {
  echo '0.5' >> /var/work/tests/result/all
