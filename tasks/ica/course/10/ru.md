...
Отдельной лабы под gRPC пока нет. Она **планируется** и должна показать то, что в тексте
не проверить на словах:
...
- опционально - `GRPCRoute` из Kubernetes Gateway API (глава 11).
