...
- **Два пути трафика.** Обращение к конкретному поду (`web-0...`) и к сервису в целом
  ...
  обычную балансировку по набору - это ожидаемо и нужно для stateful.
...
- **Сначала решите, нужен ли БД в mesh вообще.** Sidecar добавляет задержку на каждый
  ...
  БД заводят как `ServiceEntry` (глава 12), а не тянут сам StatefulSet в mesh. Заводите
  datastore в mesh осознанно, ради конкретной выгоды (mTLS, политики, наблюдаемость).
...
- Обращение к конкретному поду идёт напрямую, минуя балансировку по набору - это
  ожидаемое поведение для stateful.
