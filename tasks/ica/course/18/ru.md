...
какой процент запросов сэмплировать. Провайдер (например, `zipkin` или `jaeger`)
задаётся при установке Istio, а Telemetry ссылается на него по имени.
...
PodMonitor) или через OTel SDK и Collector (глава 18.6). Когда всё в одном хранилище,
...
- **Access logs**: включаются провайдером `envoy`; можно выборочно отключать для шумных
  сервисов.
- **Трейсинг**: провайдер + `randomSamplingPercentage`; в проде 1-5%, для отладки
  сервиса можно поднять точечно.
