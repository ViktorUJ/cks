...
Для не-HTTP трафика (TCP, БД, брокеры - глава 10) HTTP-метрик нет, но есть свои:
`istio_tcp_connections_opened_total`, `istio_tcp_connections_closed_total`,
`istio_tcp_sent_bytes_total` / `istio_tcp_received_bytes_total` - по ним смотрят соединения
и объём трафика.
...
**OpenTelemetry - текущий стандарт.** Jaeger здесь скорее «бэкенд для показа трейсов», а сам
способ их доставки индустрия унифицировала вокруг **OpenTelemetry (OTel)**: собственные
клиентские SDK Jaeger уже признаны устаревшими в пользу OTel. Istio умеет отправлять трейсы по
протоколу **OTLP** через провайдер `opentelemetry` (настраивается в MeshConfig и Telemetry
API, глава 18), а на приёме может стоять что угодно с поддержкой OTLP - Jaeger, Grafana Tempo,
облачный сервис. Часто в середине ставят **OpenTelemetry Collector**: прокси-агрегатор, куда
Envoy шлёт спаны, а он уже маршрутизирует их в один или несколько бэкендов. Практический вывод:
«Jaeger» в этой главе - про UI/хранилище, а транспорт трейсов сегодня выбирают OTLP.
...
проблемы). Если подключить к Kiali трейсинг-бэкенд (Jaeger/Tempo), он умеет показывать и
**трейсы прямо из графа** - кликнув по сервису, можно провалиться в трассировку конкретного
запроса, не переключаясь в отдельный Jaeger UI. Доступ к UI:
...
**Observability на EKS/AWS.** Если не хочется самим держать Prometheus/Grafana/Jaeger, на AWS
есть управляемые сервисы, и Istio с ними стыкуется штатно:

- **Amazon Managed Service for Prometheus (AMP)** - управляемое хранилище метрик. Свой
  Prometheus (agent-режим) или ADOT-коллектор делают `remote_write` в AMP; хранение и
  масштабирование - на стороне AWS.
- **Amazon Managed Grafana (AMG)** - управляемая Grafana с готовой интеграцией с AMP и
  X-Ray; сюда же ставят дашборды Istio.
- **AWS Distro for OpenTelemetry (ADOT)** - сборка OpenTelemetry Collector от AWS. Envoy шлёт
  метрики/трейсы по OTLP в ADOT, а он раздаёт их в AMP (метрики), **AWS X-Ray** или Tempo
  (трейсы), CloudWatch (логи).
- **Трейсинг - в AWS X-Ray** через OTLP/ADOT (вместо самостоятельного Jaeger).
- **Логи** Envoy - в **CloudWatch Logs** (через Fluent Bit / CloudWatch agent на нодах).

Доступ к AMP/AMG/X-Ray выдают через IAM (IRSA на ServiceAccount коллектора), секреты и
масштабирование - забота AWS. Это тот же принцип, что и с ACM PCA в главе 16: отдать
эксплуатацию управляемому сервису, а в кластере держать только экспортёр/коллектор.
...
- Транспорт трейсов сегодня - **OpenTelemetry/OTLP** (клиенты Jaeger устарели); Istio шлёт
  спаны по OTLP через провайдер `opentelemetry`, часто через OpenTelemetry Collector, а Jaeger
  выступает как UI/хранилище.
- Для не-HTTP трафика есть свои метрики `istio_tcp_*` (соединения, байты).
...
- На EKS observability можно отдать управляемым сервисам: **AMP** (метрики), **AMG**
  (Grafana), **ADOT** (OpenTelemetry Collector), **X-Ray** (трейсы), CloudWatch (логи); доступ
  через IRSA.
...
8. Что такое OpenTelemetry/OTLP и какова роль Jaeger при таком транспорте трейсов?
9. Какие управляемые сервисы AWS используют для observability Istio на EKS и что делает ADOT?
10. Какими метриками смотрят не-HTTP (TCP) трафик?
