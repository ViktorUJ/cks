#!/bin/bash
echo " *** worker pc ica lab 19 k8s-1"
export KUBECONFIG=/root/.kube/config

# Install istioctl on the worker PC so the student can install Istio with a
# custom CA and inspect workload certificates (istioctl proxy-config secret).
version=1.29.1
cd /tmp
