...
## 步驟 2. 部署應用程式 + Gateway/VirtualService
...
瀏覽器（選用）：`kubectl -n istio-system port-forward svc/prometheus 9090:9090`
...
瀏覽器（選用）：`kubectl -n istio-system port-forward svc/tracing 8080:80`，然後開啟 `/jaeger`
