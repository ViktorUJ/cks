...
- `SIMPLE` without `caCertificates` does not verify the server certificate. In production
  you would set `caCertificates` (and optionally `subjectAltNames`) to verify the
  upstream, or use `MUTUAL` for client-certificate authentication.
