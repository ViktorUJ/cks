#!/bin/bash
echo " *** worker pc ica lab 33 k8s-1"
export KUBECONFIG=/root/.kube/config

# Install istioctl on the worker PC so the student can run proxy-config /
# analyze and inspect the effect of discovery selectors and Sidecar scope.
version=1.29.1
cd /tmp
...
address=$(kubectl get no -o json  | jq -r '.items[] | select(.kind == "Node") | .status.addresses[] | select(.type == "InternalIP") | .address' | head -n1)
