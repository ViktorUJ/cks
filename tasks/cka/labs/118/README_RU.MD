...
| Критерии приёмки    | - Под `dns-default` (image `busybox:1.36`, `sleep`) — дефолтный DNS (`ndots:5` в resolv.conf)<br>- Под `dns-tuned` (image `busybox:1.36`, `sleep`) с `dnsConfig.options` `ndots=2` |
---
|        **5**        | **Отчёт по ndots**                                          |
| :-----------------: | :----------------------------------------------------------- |
