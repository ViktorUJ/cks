...
```mermaid
flowchart LR
    netns["network namespace<br>изолированная сеть процесса"] --> veth["veth-пара<br>кабель между namespace"]
...
подключены veth-концы всех подов ноды, поэтому поды **на одной ноде** общаются друг с
другом через bridge, как устройства в одном свитче.

```mermaid
flowchart TB
    br["bridge cni0 (коммутатор ноды)"]
    br --- p1["veth пода A → 10.244.0.5"]
    br --- p2["veth пода B → 10.244.0.6"]
...
- **network namespace** - изолированный сетевой стек процесса (свои интерфейсы, IP,
  маршруты).
...
Это последняя глава нулевого фундамента. Механизм вы увидите руками в лабе 123
(установка CNI с нуля, инспекция интерфейсов и маршрутов) и в сетевом troubleshooting
(глава 46). Дальше начинается основной курс - глава 1.

---
[Оглавление](../README_RU.md) · [Глава 0.6](../00-6-yaml/ru.md) · [Глава 1](../01/ru.md)
