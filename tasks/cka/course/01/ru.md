...
```mermaid
flowchart TB
    CP["<b>Control plane + ноды → зона CKA</b><br>kube-apiserver · etcd · scheduler<br>controller-manager · kubelet · kube-proxy"]
    Apps["<b>Приложения → зона CKAD</b><br>Deployment · Pods<br>ConfigMap / Secret · Service / Ingress"]
...
сажают в браузерный терминал с реальными кластерами и дают набор задач: что-то
создать, починить, настроить. Проктор наблюдает через камеру и экран.
...
```mermaid
flowchart TB
    P1["Ч.1 Основы 🟪"] --> P2["Ч.2 Нагрузки и планирование 🟪"]
    P2 --> P3["Ч.3 Конфигурация и безопасность 🟪"]
    P3 --> P4["Ч.4 Дизайн приложений 🟩"]
    P4 --> P5["Ч.5 Хранение 🟪"]
    P5 --> P6["Ч.6 Наблюдаемость 🟪"]
    P6 --> P7["Ч.7 Сервисы и сеть 🟪"]
    P7 --> P8["Ч.8 Архитектура и установка 🟦"]
    P8 --> P9["Ч.9 Troubleshooting 🟦"]
    P9 --> P10["Ч.10 Подготовка к экзаменам"]
    style P1 fill:#0f9d58,color:#fff
    style P2 fill:#0f9d58,color:#fff
    style P3 fill:#0f9d58,color:#fff
...
Со следующей главы начинается разбор устройства кластера, а практическая работа с
командами - с главы 3. Чтобы всё это было где отрабатывать, в первой лабораторной вы
поднимете учебный кластер и настроите доступ к нему.

🧪 Лаба 101 (основы: kubectl, поды, Deployment, Service): [tasks/cka/labs/101](../../labs/101/README_RU.MD)
