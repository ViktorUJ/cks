...
```mermaid
flowchart TB
    r1["1 · Каждый под<br>получает свой<br>уникальный IP"]
    r2["2 · Все поды могут<br>общаться друг с другом<br>БЕЗ NAT"]
    r3["3 · Каждый под<br>видит себя по тому же<br>IP, что и другие"]
    r4["4 · Агенты на ноде<br>(kubelet) могут общаться<br>с подами ноды"]
    r1 --> r2 --> r3 --> r4
...
```mermaid
flowchart LR
    pa["Под A на Ноде 1<br>10.1.1.5"] <-->|"напрямую по IP,<br>без NAT"| pc["Под C на Ноде 2<br>10.1.2.9"]
...
| **AWS VPC CNI** | поды получают реальные IP из VPC (через ENI), без оверлея; по умолчанию в EKS |
| **Azure CNI** | поды получают IP из сети VNet, нативная интеграция с сетью Azure |
| **GKE (Dataplane V2)** | управляемый CNI Google на базе Cilium/eBPF |

> **Облачные (управляемые) CNI.** В managed-кластерах (EKS, AKS, GKE) провайдер обычно
> ставит свой CNI. Показательный пример - **AWS VPC CNI** (`amazon-vpc-cni-k8s`),
> используемый в EKS по умолчанию: он не делает оверлей, а выдаёт подам **настоящие
> IP-адреса из подсети VPC**, назначая их на сетевые интерфейсы (ENI) инстансов. Плюсы -
> под виден в VPC как обычный хост, работает без инкапсуляции (быстрее) и напрямую дружит с
> Security Groups, VPC-маршрутизацией и flow logs. Расплата за это:
>
> - **поды тратят адреса VPC** - при больших кластерах реально упереться в нехватку IP в
>   подсети (нужно планировать CIDR заранее);
> - **плотность подов на ноде ограничена** числом ENI и IP на инстанс (зависит от типа
>   EC2); режим prefix delegation это ослабляет, выдавая на ENI блоки /28.
>
> Для экзамена (CKA/CKS) это знать не обязательно, но в реальной работе с EKS выбор и
> настройка CNI - одно из первых архитектурных решений. NetworkPolicy в EKS долго не
> поддерживались самим VPC CNI, поэтому его часто дополняют Calico или включают встроенную
> поддержку сетевых политик.
...
```mermaid
flowchart TB
    ov0["Оверлей (VXLAN и т.п.)"] --> o1["пакет пода<br>упаковывается в пакет<br>между нодами<br>(инкапсуляция)"] --> o2["работает поверх<br>любой сети, но есть<br>накладные расходы"]
    rt0["Маршрутизируемая (BGP)"] --> r1["сеть знает маршруты<br>к IP подов напрямую"] --> r2["быстрее, но требует<br>поддержки от сети"]
...
```mermaid
flowchart TB
    net["Сеть подов (CNI)<br>плоская,<br>под-под по IP"] --> svc["Service (kube-proxy)<br>стабильный ClusterIP<br>+ балансировка"]
    svc --> dns["DNS (CoreDNS)<br>имя сервиса →<br>ClusterIP"]
    dns --> app["Приложение<br>обращается по имени<br>backend:8080"]
