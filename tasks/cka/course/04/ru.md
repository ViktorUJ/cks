...
> **Откуда берётся общий IP (pause-контейнер).** Общий сетевой адрес пода не «выдаётся»
> контейнерам приложения напрямую - его держит скрытый служебный контейнер **pause**
> (его ещё называют infra-контейнером). Когда kubelet создаёт под, он **первым** запускает
> крошечный pause-контейнер: тот получает IP пода и удерживает сетевой namespace (а также
> IPC). Контейнеры приложения затем стартуют уже **внутри** этих namespace'ов pause -
> поэтому у всех один IP, общий `localhost` и один диапазон портов. Важное следствие:
> pause почти ничего не делает (просто «спит»), но живёт всё время жизни пода, поэтому
> перезапуск или падение контейнера приложения **не меняет IP пода** - namespace остаётся
> за pause.
>
> Увидеть это можно прямо на ноде через `crictl` (утилита CRI, глава 2):
>
> ```bash
> crictl ps            # рабочие контейнеры пода
> crictl pods          # сами поды (sandbox) — это и есть pause-контейнеры
> ```
>
> Каждому поду соответствует один pod sandbox (pause); в выводе `crictl ps` вы видите
> контейнеры приложения, а «песочницу» с сетью держит pause за кадром.
