...
> **kube-proxy не пропускает трафик через себя.** Это важно повторить (см. также главу 2):
> kube-proxy - «control plane» для правил сервисов на ноде, а не «data plane». Он лишь
> **настраивает правила ядра** (iptables/IPVS), а сам пакет от клиента к поду идёт
> **напрямую через ядро**, минуя процесс kube-proxy. На диаграмме выше это видно: стрелка
> `пакет → правила → под` не проходит через узел kube-proxy.
>
> Отсюда практическое следствие: **перезапуск или обновление kube-proxy не прерывает
> трафик.** Пока процесс перезапускается, уже настроенные в ядре правила остаются на месте
> и продолжают обслуживать существующие и новые соединения. Временно «замирает» только
> **обновление** правил - новые Service/Endpoints не появятся и удалённые не уберутся, пока
> kube-proxy снова не поднимется. Поэтому апгрейд kube-proxy (DaemonSet) - штатная
> операция без даунтайма для трафика сервисов.

> **Балансировка происходит на ноде-отправителе.** Когда под обращается к сервису по
> ClusterIP, выбор конкретного бэкенд-пода (DNAT) делают правила ядра **на той же ноде,
> где запущен под-отправитель** - потому что kube-proxy настроил одинаковые правила на
> каждой ноде. То есть решение «в какой из подов сервиса пойдёт это соединение»
> принимается локально, ещё до того как пакет покинул ноду. После подмены адреса пакет
> идёт **напрямую** по сети подов к выбранному бэкенду - хоть на этой же ноде, хоть на
> другой, без промежуточного «прокси-хопа».
>
> Практические следствия:
>
> - нет единой точки, через которую проходит весь трафик сервиса, - балансировка
>   распределена по нодам-источникам, поэтому хорошо масштабируется;
> - выбор бэкенда идёт **на уровне соединения** (L4): все пакеты одного TCP-соединения
>   попадут в один и тот же под, а новое соединение может уйти в другой;
> - по умолчанию (`externalTrafficPolicy`/`internalTrafficPolicy: Cluster`) под-получатель
>   может оказаться на любой ноде; это нормально благодаря плоской сети подов (глава 30).
...
```mermaid
flowchart LR
    pod["Под<br>curl backend:8080"] -->|"1 — резолвить<br>backend"| coredns["CoreDNS"]
    coredns -->|"2 — backend →<br>10.96.0.10"| pod
    pod -->|"3 — запрос на<br>10.96.0.10"| svc["Service<br>(kube-proxy)"]
...
flowchart TB
    q["Тип сервиса"]
    q -->|"обычный<br>ClusterIP"| a["A-запись →<br>один ClusterIP"]
    q -->|"headless<br>(clusterIP: None)"| b["A-записи →<br>IP всех подов"]
    q -->|"headless +<br>StatefulSet"| c["имя на каждый под:<br>db-0.db..."]
...
spec:
  dnsConfig:
    options:
    - name: ndots
      value: "2"                   # меньше лишних попыток для внешних имён
  containers:
  - name: web
    image: nginx
```
