...
Где эти файлы на ноде физически. Рантайм пишет реальные файлы в
`/var/log/pods/<namespace>_<pod>_<uid>/<container>/*.log`, а рядом каталог
`/var/log/containers/` содержит **симлинки** на них с удобными именами. Именно эту пару
обычно читают лог-агенты (Fluent Bit, Fluentd, Promtail), когда собирают логи со всех нод:

```mermaid
flowchart LR
    pods["/var/log/pods/<br>ns_pod_uid/<br>container/*.log<br>(реальные файлы)"]
    cont["/var/log/containers/<br>*.log<br>(симлинки)"]
    agent["лог-агент<br>Fluent Bit / Fluentd<br>на каждой ноде"]
    cont -->|"ссылаются на"| pods
    agent -->|"читает и отправляет<br>в хранилище логов"| cont
    style pods fill:#326ce5,color:#fff
    style cont fill:#0f9d58,color:#fff
    style agent fill:#f4b400,color:#000
```

Отсюда важное следствие: `kubectl logs` читает файл **текущего** контейнера на ноде, и при
удалении пода или ротации файла эти логи пропадают. Долговременное хранение обеспечивает
именно внешний агент, отправляющий логи в централизованное хранилище (глава про
Prometheus/стек логирования - ниже).

### Сколько логи живут на ноде и как это настроить

Срок жизни лога на ноде задаётся **не временем, а размером**: ротацией управляет
**kubelet**, а не приложение. Когда текущий файл дорастает до предельного размера, он
ротируется, а самые старые ротированные файлы удаляются. Значения по умолчанию:

- `containerLogMaxSize` - **10Mi** (размер файла, при котором происходит ротация);
- `containerLogMaxFiles` - **5** (сколько файлов на контейнер хранить).

То есть по умолчанию на контейнер удерживается примерно `5 × 10Mi ≈ 50Mi`, а «сколько это
в часах/днях» полностью зависит от того, насколько интенсивно приложение пишет логи:
болтливый сервис перетрёт свои старые логи за минуты, тихий - будет хранить их сутками.
Отдельного TTL по времени нет, а при удалении пода файлы убираются в любом случае.

Настраивается это в **конфигурации kubelet** (`KubeletConfiguration`, применяется при
старте kubelet на ноде):

```yaml
# /var/lib/kubelet/config.yaml (фрагмент)
apiVersion: kubelet.config.k8s.io/v1beta1
kind: KubeletConfiguration
containerLogMaxSize: "50Mi"   # ротация при 50 МиБ
containerLogMaxFiles: 5        # хранить до 5 файлов на контейнер
```

Старые флаги `--container-log-max-size` и `--container-log-max-files` делают то же, но
считаются устаревшими - предпочтителен файл конфигурации. Практическое правило: суммарный
объём (`containerLogMaxSize × containerLogMaxFiles`) на контейнер держат небольшим (обычно
до ~1% диска ноды), чтобы логи не забили диск и не вызвали disk-pressure eviction (глава 15).
...
flowchart LR
    ...
    restart --> now["kubectl logs →<br>логи НОВОГО<br>(причина не видна)"]
    restart --> prev["kubectl logs --previous →<br>логи УПАВШЕГО<br>(вот причина!)"]
...
> **А если под перезапускался много раз и нет централизованного хранилища?** `--previous`
> отдаёт логи только **одного** предыдущего запуска (последнего перед текущим), более
> ранние через `kubectl logs` не достать. Но на ноде их часто можно найти напрямую: каждый
> перезапуск контейнера кладёт отдельный файл в
> `/var/log/pods/<namespace>_<pod>_<uid>/<container>/`, названный по счётчику
> перезапусков - `0.log`, `1.log`, `2.log` и т.д. (старые вдобавок сжаты ротацией). Значит
> логи нескольких прошлых падений могут лежать там, пока их не вытеснила ротация.
>
> Добраться до этих файлов, не заходя по SSH, помогает отладочный под на ноде:
>
> ```bash
> kubectl debug node/<node> -it --image=busybox
> # внутри: файловая система ноды смонтирована в /host
> ls /host/var/log/pods/<namespace>_<pod>_<uid>/<container>/
> cat /host/var/log/pods/<namespace>_<pod>_<uid>/<container>/1.log
> ```
>
> Либо на самой ноде - через рантайм: `crictl ps -a` (найти ID) и `crictl logs <id>`.
>
> Важные ограничения: файлы привязаны к **UID пода** - если под **удалён** (а не просто
> перезапущен), весь каталог с логами исчезает; ротация хранит лишь последние
> `containerLogMaxFiles` файлов; а если под переехал на другую ноду, искать надо на
> прежней. Поэтому node-local логи - лишь временная страховка: единственный надёжный способ
> не терять историю падений - централизованный сбор логов (агент → внешнее хранилище).
