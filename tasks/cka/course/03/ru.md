...
## 3.2. Как установить kubectl

На экзамене и в наших лабах `kubectl` уже установлен - ставить его самому не нужно. Но
для тренировки на своей машине его надо поставить и, что важнее, понимать **правило
совместимости версий**.

> **Правило skew (расхождения версий).** Версия `kubectl` должна отличаться от версии
> `kube-apiserver` не более чем на **один минорный релиз** (в обе стороны). Например, к
> API-серверу 1.34 подойдут `kubectl` 1.33, 1.34 или 1.35, но не 1.32 или 1.36. На
> практике держите `kubectl` той же минорной версии, что и кластер.

Способы установки под разные ОС:

| ОС / менеджер | Команда |
|---------------|---------|
| Linux (бинарник) | `curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl"` |
| Linux (apt, Debian/Ubuntu) | `sudo apt-get install -y kubectl` (после подключения репозитория pkgs.k8s.io) |
| Linux (dnf, RHEL/Fedora) | `sudo dnf install -y kubectl` (после подключения репозитория) |
| macOS (Homebrew) | `brew install kubectl` |
| Windows (choco) | `choco install kubernetes-cli` |

Ручная установка бинарника на Linux целиком:

```bash
# 1. Скачать бинарник последней стабильной версии
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl"

# 2. (опционально) проверить контрольную сумму
curl -LO "https://dl.k8s.io/release/$(curl -L -s https://dl.k8s.io/release/stable.txt)/bin/linux/amd64/kubectl.sha256"
echo "$(cat kubectl.sha256)  kubectl" | sha256sum --check

# 3. Установить в PATH с нужными правами
...
```bash
kubectl version --client            # версия только клиента (без обращения к кластеру)
kubectl version                     # версии клиента и сервера (нужен доступ к кластеру)
```

> **Совет для экзамена.** Тратить время на установку не придётся - окружение готово:
> `kubectl`, алиас `k` и автодополнение уже настроены из коробки. Своё окружение под
> установку и настройку (раздел 3.10) имеет смысл готовить только для тренировки на
> личной машине.

## 3.3. Анатомия команды kubectl
...
## 3.4. Два подхода: императивный и декларативный
...
```mermaid
flowchart TB
    subgraph IMP["Императивный подход"]
        direction TB
        i1["kubectl run nginx<br>--image=nginx"] --> i2["kubectl scale ...<br>--replicas=3"] --> i3["kubectl delete<br>po nginx"]
    end
    subgraph DEC["Декларативный подход"]
        direction TB
        d1["пишем<br>pod.yaml"] --> d2["kubectl apply<br>-f pod.yaml"] --> d3["правим yaml,<br>снова apply"]
    end
...
## 3.5. Императивные команды: создаём объекты быстро
...
## 3.6. Генерация манифестов: `--dry-run=client -o yaml`
...
```mermaid
flowchart TB
    gen["kubectl run/create ...<br>--dry-run=client -o yaml"] -->|"вывод в pod.yaml"| file["pod.yaml (каркас)"]
    file -->|"vim: дописать пробы,<br>тома, ресурсы"| edit["готовый pod.yaml"]
...
## 3.7. Декларативный подход: apply, create, replace
...
## 3.8. Читаем состояние: get, describe, logs
...
## 3.10. Настройка окружения для скорости

На актуальном экзамене (PSI) базовое окружение уже готово из коробки: `kubectl`, алиас
`k` и автодополнение обычно преднастроены - специально что-то устанавливать не нужно.
Поэтому первым делом на экзамене стоит не настраивать окружение, а **проверить**, что
нужное уже работает (`k get ns`, автодополнение по `Tab`). А переменные-хелперы
(`do`, `now`) по умолчанию не заданы - их при желании добавляете сами.

Для тренировки на своей машине весь набор ниже настраивают самостоятельно - он экономит
десятки минут.
...
## 3.11. `kubectl explain`: документация прямо в терминале
...
## 3.12. Редактирование и удаление живых объектов
...
## 3.13. Как это применяют в продакшене
...
- На экзамене базовое окружение (`kubectl`, алиас `k`, автодополнение) обычно
  преднастроено - проверьте это, а не настраивайте с нуля; хелперы `do`/`now` при
  желании добавляете сами. Для своей тренировочной машины весь набор (алиас, `do`/`now`,
  автодополнение, vim с 2 пробелами) настройте сами - он экономит десятки минут.
- `kubectl explain` заменяет поход в браузер за именами полей.

## 3.16. Как это пригодится: на экзамене и в реальной работе
