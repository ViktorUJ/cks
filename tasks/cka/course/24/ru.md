...
flowchart TB
    q["Проблема"]
    q -->|"данные пропадают<br>при рестарте контейнера"| p1["том переживает<br>рестарт контейнера"]
    q -->|"контейнеры пода<br>не видят файлы друг друга"| p2["общий том виден<br>нескольким контейнерам"]
    q -->|"нужны данные,<br>переживающие сам под"| p3["постоянные тома<br>(PV/PVC, глава 25)"]
...
flowchart TB
    ...
    del["Под удалён →<br>emptyDir и все<br>данные исчезли"]
    Pod -.-> del
...
flowchart LR
    ...
    warn["⚠️ привязка к ноде +<br>риск безопасности:<br>под видит файлы хоста"]
    node -.-> warn
...
## 24.7. Практический кейс: создать, посмотреть, смонтировать, удалить

Разберём полный цикл работы с эфемерным томом на примере emptyDir, общего для двух
контейнеров пода.

**1. Создать под с томом и смонтировать в два контейнера.**
...
**2. Посмотреть тома пода.**

```bash
# том и точки монтирования — в describe (секции Volumes и Mounts)
kubectl describe pod shared-vol

# только объявленные тома из спеки
kubectl get pod shared-vol -o jsonpath='{.spec.volumes}'

# что реально примонтировано внутри контейнера
kubectl exec shared-vol -c writer -- df -h /data
kubectl exec shared-vol -c writer -- mount | grep /data
```

**3. Проверить, что том общий.** Файл, записанный `writer`, виден `reader`:

```bash
kubectl exec shared-vol -c reader -- cat /data/msg   # hello
```

Так как `reader` смонтировал том с `readOnly: true`, запись из него упадёт с ошибкой
«read-only file system» - удобно, когда потребитель не должен менять данные.

**4. «Удалить» том.** Отдельной команды удаления эфемерного тома нет - он живёт вместе с
подом. Убрать том можно двумя способами:

- убрать `volumes` и соответствующие `volumeMounts` из манифеста и применить
  (`kubectl apply -f shared-vol.yaml`) - под пересоздаётся уже без тома;
- удалить сам под - `kubectl delete pod shared-vol` - вместе с ним исчезает emptyDir и
  все данные.

Чтобы убедиться, что данные эфемерны: удалите и пересоздайте под, затем проверьте -
`/data/msg` уже пуст, emptyDir создаётся заново.

### Возможности по размеру и расширению

- у emptyDir есть только `sizeLimit` - верхняя граница объёма. Превышение приводит к
  вытеснению пода (evicted), а не к автоматическому росту.
- **расширить эфемерный том «на лету» нельзя.** Поля тома у запущенного пода иммутабельны:
  чтобы изменить `sizeLimit` или `medium`, под нужно пересоздать (правка манифеста +
  `kubectl apply`, под пересоздаётся).
- **онлайн-расширение - свойство постоянных томов.** У PVC при `allowVolumeExpansion:
  true` в StorageClass можно увеличить запрошенный размер без пересоздания пода
  (главы 25-26). У emptyDir/configMap/secret такого механизма нет.
- отдельно стоят **generic ephemeral volumes** (`spec.volumes[].ephemeral` с шаблоном
  PVC): они эфемерны по времени жизни (удаляются с подом), но опираются на PVC и потому
  наследуют его правила, включая расширение. Это гибрид на стыке с главой 25.

## 24.8. Как это применяют в продакшене
...
- Тома пода смотрят через `kubectl describe pod` (Volumes/Mounts) и `kubectl exec ... df/mount`;
  отдельной команды удаления эфемерного тома нет - он уходит с подом.
- Эфемерный том нельзя расширить «на лету» (поля иммутабельны, нужно пересоздание пода);
  онлайн-расширение есть только у PVC (`allowVolumeExpansion`, глава 25-26).

## 24.11. Как это пригодится: на экзамене и в реальной работе
...
## 24.12. Вопросы для самопроверки
...
8. Как посмотреть тома и точки монтирования пода и как «удалить» эфемерный том?
9. Можно ли расширить emptyDir у запущенного пода и где онлайн-расширение вообще доступно?
