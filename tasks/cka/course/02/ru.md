...
- **Worker-ноды (рабочие узлы)** - «мышцы» кластера. Именно на них запускаются ваши
  контейнеры с приложениями. На схеме показана одна worker-нода, но в реальном кластере
  их обычно несколько (от единиц до сотен) - все они устроены одинаково и подключены к
  control plane через API-сервер.

```mermaid
flowchart TB
    subgraph CP["<b>Control plane (мозг)</b>"]
        api["kube-apiserver"]
        etcd[("etcd")]
    ...
    w1["<b>Worker-нода (мышцы)</b><br>──────────<br>kubelet · kube-proxy<br>поды приложений<br>(таких нод может быть много)"]
    api <-->|"регистрация,<br>команды,<br>статусы"| w1
...
> **Важно (частое заблуждение).** С хранилищем `etcd` напрямую работает **только**
> `kube-apiserver`. Остальные компоненты (scheduler, controller-manager, kubelet,
> kube-proxy) в etcd **не ходят** - они читают и пишут состояние через API-сервер. etcd
> - это не шина обмена между компонентами, а бэкенд-хранилище за единственной «дверью» в
> лице apiserver. Это прямо следует из официальной документации: etcd описан как
> хранилище «для всех данных API-сервера»
> ([Kubernetes Components](https://kubernetes.io/docs/concepts/overview/components/)), а в
> HA-топологии член etcd «общается только с kube-apiserver» своей ноды
> ([HA topology](https://kubernetes.io/docs/setup/production-environment/tools/kubeadm/ha-topology/)).
>
> **Тогда как scheduler узнаёт о новых подах?** Не из etcd. Компоненты **подписываются**
> на изменения через API-сервер - механизм **watch** (list-watch). Когда создаётся под,
> apiserver сохраняет его в etcd и тут же рассылает событие подписчикам. Scheduler видит
> «появился под без `nodeName`», выбирает ноду и пишет решение (binding) **обратно через
> apiserver**; apiserver сохраняет это в etcd и уведомляет kubelet нужной ноды - тот тоже
> узнаёт о поде через свой watch. Так весь обмен идёт через apiserver, а etcd остаётся за
> ним. Механизм watch подробнее разберём в главе 3.
>
> **Откуда взялся миф.** У него есть исторический корень: в ранних версиях Kubernetes (до
> 1.0, 2014-2015) компоненты действительно ходили в etcd напрямую - kubelet читал свои
> поды из etcd, а scheduler назначал их через etcd-примитивы (`CompareAndSwap`, watch по
> ключу). К релизу 1.0 архитектуру сознательно консолидировали: apiserver стал
> единственной «дверью» к etcd (централизованные auth/RBAC/admission, развязка
> компонентов, единый источник правды), и все перешли на watch API-сервера. Миф живёт и
> потому, что на многих схемах etcd рисуют в центре control plane - визуально похоже на
> «шину», хотя это лишь хранилище за apiserver.
...
Важный момент: **сам трафик через kube-proxy не идёт**. Он не стоит на пути пакетов, а
лишь *настраивает* правила ядра (iptables/IPVS), по которым трафик потом идёт **напрямую**,
уже без участия kube-proxy. То есть kube-proxy - это «control plane» для правил сервисов на
ноде, а не «data plane». Отсюда важное следствие для эксплуатации:

- Если kube-proxy **упал** - уже настроенные правила остаются в ядре и **продолжают
  работать**: существующие сервисы доступны, трафик с подов этой ноды не прерывается.
  Ломается лишь **обновление** правил - новые Service/Endpoints не добавляются, удалённые
  не убираются, пока kube-proxy не поднимется снова.
- Поэтому **перезапуск или обновление версии** kube-proxy на ноде проходит незаметно для
  трафика: пока новый под стартует, старые правила действуют, и соединения не рвутся.

- **Что делает:** настраивает правила iptables/IPVS для Service на ноде (трафик идёт мимо него).
- **Где живёт:** обычно DaemonSet в namespace `kube-system` (`kubectl get ds -n kube-system`).
- **Если упал:** существующие правила работают, сервисы доступны; перестают применяться
  только изменения (новые/удалённые Service и Endpoints) до его восстановления.
...
sequenceDiagram
    ...
    A-->>S: 4. (watch) новый под без ноды: nginx
    S->>A: 5. Назначить под на ноду-2
    A->>E: записать nodeName = нода-2
    A-->>K: 6. (watch) ноде-2 назначен под nginx
...
не от `kubectl` и не опрашивая кого-то - он **подписан** на API-сервер через watch, и
apiserver **сам** прислал ему событие «появился под без ноды». kubelet узнал о своём поде
так же - через watch на API-сервере (apiserver уведомил его, когда под назначили на эту
ноду). Каждый шаг - это запись или чтение через единственную дверь, а уведомления идут
событиями watch (детали - в 2.6). Именно так работает вся
...
```mermaid
flowchart LR
    c["Компонент (scheduler,<br>controller-manager, kubelet)"] -->|"LIST:<br>состояние"| api["API-сервер"]
    c -->|"WATCH:<br>подписка"| api
    api -->|"события:<br>add / update / delete"| c
    api --- etcd["etcd (watch)"]
...
> **Как watch устроен на сетевом уровне.** Это не мультикаст и не поллинг, а обычное
> **unicast-соединение поверх TCP/TLS по HTTP** (по умолчанию HTTP/2). Клиент открывает
> один долгоживущий запрос (`GET ...?watch=true`), а API-сервер **не закрывает ответ** и
> **стримит** в него события - объекты `WatchEvent` (`ADDED`/`MODIFIED`/`DELETED`/
> `BOOKMARK`) построчно. У каждого клиента своё соединение: apiserver сам «смотрит» etcd,
> держит изменения в памяти (**watch cache**) и **рассылает** их всем подключённым
> клиентам (fan-out), учитывая RBAC и селекторы - поэтому мультикаст и не нужен (он не дал
> бы ни TLS/авторизации, ни надёжности, ни фильтрации на каждого клиента). При обрыве
> клиент переоткрывает watch с сохранённого `resourceVersion` и не теряет изменения, а
> периодические события `BOOKMARK` двигают эту версию вперёд.
...
вживую чуть позже: в готовом кластере можно будет заглянуть в
`/etc/kubernetes/manifests/` и проверить статусы компонентов control plane, а собрать
кластер с нуля своими руками (`kubeadm init` + CNI + `join`) - в главе 35, когда
разберём установку.
