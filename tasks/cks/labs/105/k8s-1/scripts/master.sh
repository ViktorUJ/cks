...
# Exclude /var/lib/containerd - its overlayfs snapshot directories are ephemeral
# container image layer contents, not host-installed binaries. Their exact paths
# (snapshot ID) change whenever ANY container image is pulled/extracted during the lab
# session (e.g. a Pod recreated elsewhere in this same lab), which would otherwise make
# this baseline diverge from a correct student's later state through no action of their
# own - this task is about host SUID binaries (sudo, passwd, mount, ...), not container
# image layer contents sitting in overlayfs.
find / -xdev -perm -4000 -type f -print 2>/dev/null \
  | grep -v '^/var/lib/containerd/' \
  | sort \
