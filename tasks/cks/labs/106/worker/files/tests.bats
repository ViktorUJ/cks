...
@test "1. AppArmor profile is transferred to the workload node and loaded into the kernel" {
  echo '1' >> /var/work/tests/result/all
  node=$(kubectl get nodes --context "$CTX" -l "${NODE_LABEL}=true" -o jsonpath='{.items[0].metadata.name}' 2>/dev/null) || true
  # This lab no longer pre-places the profile on the node or in its kernel - the
  # student must transfer /opt/lab106-fixtures/${AA_PROFILE} from the worker and load
  # it themselves. Any loaded mode (complain or enforce) satisfies this task; task 2
...
@test "2. AppArmor Localhost profile is loaded in enforce mode on the labelled workload node" {
  echo '1' >> /var/work/tests/result/all
...
@test "3. apparmor-writer has the effective Localhost profile and receives an AppArmor write denial" {
  echo '1' >> /var/work/tests/result/all
  pod=$(kubectl get pod apparmor-writer -n "$NS" --context "$CTX" -o json 2>/dev/null) || true
  profile=$(jq -r '.spec.securityContext.appArmorProfile.type + ":" + (.spec.securityContext.appArmorProfile.localhostProfile // "")' <<<"$pod" 2>/dev/null) || true
  node_selector=$(jq -r ".spec.nodeSelector[\"${NODE_LABEL}\"] // \"\"" <<<"$pod" 2>/dev/null) || true
  automount=$(jq -r '.spec.automountServiceAccountToken' <<<"$pod" 2>/dev/null) || true
  phase=$(jq -r '.status.phase' <<<"$pod" 2>/dev/null) || true
...
@test "4. runtime-default explicitly uses RuntimeDefault seccomp and has filter mode enabled" {
  echo '1' >> /var/work/tests/result/all
  pod=$(kubectl get pod runtime-default -n "$NS" --context "$CTX" -o json 2>/dev/null) || true
  seccomp=$(jq -r '.spec.securityContext.seccompProfile.type' <<<"$pod" 2>/dev/null) || true
  node_selector=$(jq -r ".spec.nodeSelector[\"${NODE_LABEL}\"] // \"\"" <<<"$pod" 2>/dev/null) || true
  phase=$(jq -r '.status.phase' <<<"$pod" 2>/dev/null) || true
...
@test "5. localhost-seccomp uses the transferred custom Localhost seccomp profile" {
  echo '1' >> /var/work/tests/result/all
  pod=$(kubectl get pod localhost-seccomp -n "$NS" --context "$CTX" -o json 2>/dev/null) || true
  profile=$(jq -r '.spec.securityContext.seccompProfile.type + ":" + (.spec.securityContext.seccompProfile.localhostProfile // "")' <<<"$pod" 2>/dev/null) || true
  node_selector=$(jq -r ".spec.nodeSelector[\"${NODE_LABEL}\"] // \"\"" <<<"$pod" 2>/dev/null) || true
  capabilities=$(jq -c '(.spec.containers[0].securityContext.capabilities.add // []) | sort' <<<"$pod" 2>/dev/null) || true
  ...
  if [[ "$profile" == "Localhost:${SECCOMP_PROFILE}" && "$node_selector" == "true" && "$capabilities" == '["SYS_ADMIN"]' && "$node_profile" == "true" ]]; then
    ...
    if [[ "$profile" != "Localhost:${SECCOMP_PROFILE}" ]]; then
      ...
      echo "HINT: The profile JSON at /var/lib/kubelet/seccomp/${SECCOMP_PROFILE} is missing or malformed on the node. This lab does not pre-place it there - copy /opt/lab106-fixtures/cks-106-deny-unshare.json from the worker to that exact path yourself (creating /var/lib/kubelet/seccomp/profiles/ with sudo if needed) before referencing it from the Pod spec. It must have defaultAction SCMP_ACT_ALLOW and a specific rule blocking syscall 'unshare' with SCMP_ACT_ERRNO/errnoRet 1."
    fi
...
@test "6. Custom seccomp blocks unshare while an Unconfined SYS_ADMIN control can execute it" {
  echo '1' >> /var/work/tests/result/all
  phase=$(kubectl get pod localhost-seccomp -n "$NS" --context "$CTX" -o jsonpath='{.status.phase}' 2>/dev/null) || true
...
@test "7. userns-probe runs with hostUsers: false and root is translated to an unprivileged host UID" {
  echo '1' >> /var/work/tests/result/all
  pod=$(kubectl get pod userns-probe -n "$NS" --context "$CTX" -o json 2>/dev/null) || true
  phase=$(jq -r '.status.phase // ""' <<<"$pod" 2>/dev/null) || true
  host_users=$(jq -r '.spec.hostUsers' <<<"$pod" 2>/dev/null) || true
  node_selector=$(jq -r ".spec.nodeSelector[\"${NODE_LABEL}\"] // \"\"" <<<"$pod" 2>/dev/null) || true
  container_id=$(jq -r '.status.containerStatuses[]? | select(.name == "app") | .containerID' <<<"$pod" 2>/dev/null | sed 's#^containerd://##') || true
  ...
  if [[ "$phase" == "Running" && "$host_users" == "false" && "$node_selector" == "true" \
    ...
    && grep -Eq '^[[:space:]]*0[[:space:]]+[1-9][0-9]*[[:space:]]+[0-9]+$' "$artifact" 2>/dev/null; then
    echo '1' >> /var/work/tests/result/ok
    result=0
  else
...
@test "8. seccompDefault: true on kubelet applies RuntimeDefault when no profile is set" {
  echo '1' >> /var/work/tests/result/all
...
@test "9. Broken AppArmor profile syntax is fixed and loaded via apparmor_parser -r -v" {
  echo '1' >> /var/work/tests/result/all
