...
dependency "ssh-keys" { config_path = "../ssh-keys" }
...
inputs = {
  ...
  k8s_master = {
    ...
    eip                = "false"
    utils_enable       = "false"
    task_script_url    = "https://raw.githubusercontent.com/ViktorUJ/cks/refs/heads/master/tasks/cks/labs/113/k8s-1/scripts/master.sh"
  ...
  # Один worker-узел на той же стартовой версии, что и control-plane - upgrade лаба
  # требует апгрейда обоих типов узлов в правильном порядке (control-plane первым).
  k8s_worker = {
    "worker1" = {
      k8_version         = local.vars.locals.k8_version
      ...
      runtime            = local.vars.locals.runtime
      runtime_script     = "template/runtime.sh"
      task_script_url    = "https://raw.githubusercontent.com/ViktorUJ/cks/refs/heads/master/tasks/cks/labs/113/k8s-1/scripts/worker.sh"
      node_labels        = "lab.cks.io/role=upgrade-target"
      cidrs              = local.vars.locals.access_cidrs
      root_volume        = local.vars.locals.root_volume
      ssh = {
        private_key = dependency.ssh-keys.outputs.private_key
