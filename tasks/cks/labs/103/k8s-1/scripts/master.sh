...
# Стартовая уязвимость для задания 8: тот же паттерн, что и в задании 7, но для другого
# control-plane компонента и другого check ID (CIS 1.1.3 - kube-controller-manager pod
# specification file permissions, requirement <= 600).
...
# Some kubeadm/kubelet versions omit readOnlyPort entirely from the serialized
# KubeletConfiguration when it equals the zero-value default (0), instead of always writing
# it out - so it may not be there to sed-replace at all. Handle both cases explicitly.
if grep -qE '^readOnlyPort:' "$KUBELET_CONFIG"; then
  sed -i 's/^readOnlyPort:.*/readOnlyPort: 10255/' "$KUBELET_CONFIG"
else
  echo "readOnlyPort: 10255" >> "$KUBELET_CONFIG"
fi
