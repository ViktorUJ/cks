...
@test "2. kubelet disables read-only port and anonymous authentication on the node" {
  ...
  # /configz omits readOnlyPort entirely (null, not 0) once it's back at the zero-value
  # default on this kubelet version - kubeadm/kubelet no longer always serialize it. null
  # and 0 are the same effective (disabled) state, so treat a missing field as 0.
  configz_ok=$(printf '%s' "$configz" | jq -e '
    (.kubeletconfig.readOnlyPort // 0) == 0 and
...
@test "4. TLS Secret and secure Ingress are configured" {
  ...
  https_ok=false
  [[ "$https_status" == "200" && "$https_body" == *"Server Name: secure-app"* ]] && https_ok=true
  # ingress-nginx's ssl-redirect Location does not reliably include the request path/
  # trailing slash (observed live: a bare "https://secure.cks.local" with no path at all
  # for a request to "/") - match on the host, not on an exact path suffix.
...
@test "8. kube-bench check 1.1.3 goes from FAIL to PASS after fixing kube-controller-manager manifest permissions" {
  before=/var/work/tests/artifacts/8/before.txt
  after=/var/work/tests/artifacts/8/after.txt
  perm=$(node_ssh "sudo stat -c '%a' /etc/kubernetes/manifests/kube-controller-manager.yaml" 2>/dev/null)
  phase=$(kubectl get pods -n kube-system --context "$CTX" -l component=kube-controller-manager -o jsonpath='{.items[0].status.phase}' 2>/dev/null)

  before_check_ok=false
  grep -Eq '^\[FAIL\][[:space:]]+1[.]1[.]3([[:space:]]|$)' "$before" && before_check_ok=true
  after_pass_ok=false
  grep -Eq '^\[PASS\][[:space:]]+1[.]1[.]3([[:space:]]|$)' "$after" && after_pass_ok=true
  after_no_fail=true
  ...
  if [[ -s "$before" && "$before_check_ok" == "true" ]] \
    ...
    if [[ ! -s "$before" || "$before_check_ok" != "true" ]]; then
      echo "HINT: before.txt must be captured BEFORE any fix and contain a line starting with exactly '[FAIL] 1.1.3' - this is the CIS check for kube-controller-manager manifest file permissions."
    elif [[ "$perm" != "600" ]]; then
    ...
    echo "Expected exact '[FAIL] 1.1.3' in $before and exact '[PASS] 1.1.3' (no '[FAIL] 1.1.3') in $after, node permission exactly 600 (actual: ${perm:-missing}), and Running kube-controller-manager (phase=$phase)"
    result=1
  fi
...
@test "9. kube-controller-manager and kube-scheduler have profiling disabled and stay healthy" {
  ...
  if [[ "$cm_status" -eq 0 && "$sched_status" -eq 0 && "$cm_phase" == "Running" && "$sched_phase" == "Running" ]]; then
    ...
    if [[ "$cm_status" -ne 0 ]]; then
      ...
      echo "HINT: kube-scheduler Pod is not Running (phase=$sched_phase) after the manifest edit. Check 'kubectl describe pod' for a YAML syntax error."
    fi
    echo "cm_command=$cm_command sched_command=$sched_command cm_phase=$cm_phase sched_phase=$sched_phase"
    result=1
  fi
