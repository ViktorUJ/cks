...
@test "5. Cross-node traffic reaches gVisor service while captured WireGuard traffic hides the marker" {
  ...
  if [[ -n "$remote_ip" ]]; then
    ...
    ssh -o BatchMode=yes k8s110_controlPlane_1 "sudo timeout 15 tcpdump -nni any 'host $remote_ip' -A" >"$baseline_capture" 2>&1 &
    baseline_pid=$!
    sleep 3
  ...
  if [[ -n "$remote_ip" ]]; then
    ssh -o BatchMode=yes k8s110_controlPlane_1 "sudo timeout 15 tcpdump -nni any 'host $remote_ip' -A" >"$checker_capture" 2>&1 &
  else
  ...
  if [[ "$request_status" -eq 0 && "$response" == *"CKS110-WIREGUARD-MARKER"* && "$checker_capture_ok" -eq 1 && "$baseline_capture_ok" -eq 1 ]] \
    ...
    echo "request_status=$request_status response=$response baseline_capture_ok=$baseline_capture_ok checker_capture_ok=$checker_capture_ok remote_ip=${remote_ip:-missing}; capture must show WireGuard UDP and no plaintext marker"
    result=1
  fi
