...
> **Что дальше.** Нативные NetworkPolicy уже позволяют изолировать pod'ы и закрывать
...
> **Что нужно из CKA.** Базовую модель CNI, IP pod'ов и сервисов см. в
...
Нативная `NetworkPolicy` описывает сетевые отношения на уровнях L3/L4: какие pod'ы,
...
идентичности workload'ов, L7-прокси и наблюдаемость.
...
Cilium оценивает политику по identity, а не только по IP. Для Kubernetes workload'ов
identity строится из labels. При пересоздании pod'а его IP меняется, но правило с
...
namespace. CCNP сильнее по последствиям: ошибка в широком selector'е может отрезать целый
...
Политика становится применимой к endpoint'у, если его выбирает `endpointSelector`. Когда
для направления есть применимая Cilium policy, действует allow-list: разрешён только трафик,
который совпал хотя бы с одним правилом. Поэтому selector должен быть точным.
...
следует за пересозданием workload'а и может включить чужие IP. `toCIDR` оправдан для
стабильных внешних сетей или узких служебных диапазонов, а не как обычный способ связать
два сервиса Kubernetes.

## 06.3. L7: ограничить HTTP, Kafka и DNS

L7-правило добавляется внутрь элемента `toPorts`. Cilium направляет выбранный трафик через
соответствующий L7-proxy: HTTP, Kafka или DNS. Важное следствие: L7-правила применимы только
...
Проверяйте не только успешный запрос, но и запрет. В образе тестового pod'а должны быть
...
Cilium также умеет фильтровать Kafka по API key/topic и DNS по имени запроса. Это полезно
для специализированных протоколов, но не включайте L7-proxy без нужды: он добавляет
обработку на пути трафика и требует отдельного нагрузочного тестирования.

## 06.4. DNS-aware egress и `toFQDNs`

IP публичного SaaS-сервиса меняются, CDN отдаёт разные адреса, а приложение обычно знает
не IP, а имя. `toFQDNs` разрешает egress к именам, сопоставляя их с IP, которые Cilium
увидел в DNS-ответах. Это не статический DNS-resolve во время применения YAML: Cilium
поддерживает кэш IP с учётом DNS TTL.
...
`matchPattern`, например `"*.example.com"`, но не используйте `"*"`: это фактически
возвращает неограниченный внешний egress. Перед применением проверьте реальные labels
...
доступа по наблюдаемому DNS-разрешению. Не допускайте обхода через неразрешённые DNS-серверы,
DoH/DoT или прямой IP, если для модели угроз это существенно. Ограничьте egress до DNS,
включите нужную DNS visibility и сочетайте правила с proxy/firewall на границе сети.
...
| `kube-apiserver` | Kubernetes API server | ограничить доступ workload'ов к API |
...
endpoint'ам, выбранным политикой. `egressDeny` имеет приоритет над разрешающими правилами,
поэтому исключение не получится случайно открыть другой CNP. Сначала оцените, нужны ли
metadata вызовы системным workload'ам, и при необходимости исключите их отдельным selector'ом
или namespace.
...
1. Убедитесь, что `frontend` и `backend` Running и их labels совпадают с selector'ами.
2. Примените L3/L4 CNP. Из frontend запрос к backend:80 должен пройти; из pod'а без
...
# Выполняется в pod'е cilium на выбранной ноде.
...
| `GET` и `POST` оба запрещены | L3 identity либо L4-порт не совпали | labels endpoint'ов, порт Service и targetPort |
...
Безопасный порядок: инвентаризировать текущие соединения через Hubble, добавить narrow
allow для реального трафика, проверить из тестового pod'а, затем включать deny или расширять
scope. Не начинайте с `endpointSelector: {}` в CCNP на production-кластере. Для каждого
изменения нужен rollback: `kubectl delete ciliumnetworkpolicy <name> -n <namespace>` или
откат через GitOps, а не ручная правка без истории.

## 06.8. Как это применяют в продакшене

- **Политики хранят рядом с workload'ом.** CNP для приложения проходят code review,
...
- **Labels - контракт безопасности.** Команды фиксируют labels вроде `app`, `component`,
  `tenant` и не позволяют workload'у произвольно менять security-значимые labels. Иначе
...
- **Identity** - идентификатор endpoint'а, построенный Cilium из labels.
- **L3/L4** - сетевой уровень и транспортный протокол/порт.
- **L7** - протокольный уровень, например HTTP method/path, Kafka или DNS.
...
2. Что произойдёт с ingress endpoint'а, если его выбирает CNP, но трафик не совпал ни с
