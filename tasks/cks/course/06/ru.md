...
Политика становится применимой к endpoint, если его выбирает `endpointSelector`. Когда
...
> **Актуальность.** L7-фильтрация Kafka в Cilium помечена deprecated и удалена в новых
> версиях Cilium. Для CKS ориентируйтесь на L7 HTTP и DNS/`toFQDNs`, а Kafka-политику
...
endpoint, выбранным политикой. `egressDeny` имеет приоритет над разрешающими правилами,
поэтому исключение не получится случайно открыть другой CNP. Сначала оцените, нужны ли
metadata вызовы системным рабочим нагрузкам, и при необходимости исключите их отдельным селектором
или namespace.
...
allow для реального трафика, проверить из тестового Pod, затем включать deny или расширять
