...
> **Проблема.** Скомпрометированный frontend может использовать разрешённый TCP-доступ
> к backend для `POST /admin` или отправить данные на внешний IP после DNS-resolve:
> L3/L4 NetworkPolicy этого не различает. Без L7-, FQDN- и identity-aware ограничений
> допустимое соединение превращается в канал для опасного запроса или эксфильтрации,
> а отсутствие наблюдаемости затрудняет обнаружение DROP и расследование.
...
🧪 Лаба 115 (установка Cilium с нуля: замена kube-proxy, WireGuard и Mutual Authentication на SPIRE - advanced/production track, не входит в формальный CKS Core exam requirement): [tasks/cks/labs/115](../../labs/115/README_RU.MD)
