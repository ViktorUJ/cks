...
> **Проблема.** На CKS правильная настройка не принесёт баллы, если применить её не на том
> SSH-хосте, в неверном context или namespace либо не проверить фактический результат.
> Два часа и несколько практических задач усиливают цену долгого поиска, рискованной правки
> static Pod и перехода к следующему заданию со сломанным кластером. Нужен повторяемый
> workflow: scope, минимальное изменение, evidence, проверка и возврат на `base`.
...
CKS - performance-based экзамен: проверяется состояние живого кластера, ноды и созданных артефактов, а не текст ответа. На дату проверки **2026-09-05** страница продукта LF указывает Kubernetes `v1.35` для экзамена. `v1.36` - целевая версия курса и production-расширение, а не обещание для CKS. Curriculum PDF и другие документы могут обновляться в иной момент, поэтому непосредственно перед экзаменом заново сверьте страницу продукта LF, Important Instructions, Resources Allowed и ExamUI. Версия Kubernetes, веса доменов, разрешённые ресурсы, сочетания клавиш и параметры simulator — high-churn snapshots: при расхождении сохранённого текста с фактическим ExamUI/инструкциями на дату экзамена приоритет имеют ExamUI и актуальные инструкции LF.

> 🎯 Разделы 33.1–33.6 — единый exam workflow: на `base` прочитайте условие, подключитесь к назначенному host, подтвердите context и scope, внесите минимальное изменение, докажите результат и вернитесь на `base`. Используйте разрешённую документацию для точного поля или флага, распределяйте время через флаги задач и в финале перепроверяйте каждый критерий.
...
По странице продукта LF включённый симулятор даёт **две попытки**. Каждая попытка содержит **17 сценариев**, после активации доступна **36 часов** и использует другой набор из 17 сценариев с оценённым результатом. Число 17 и длительность окна — snapshot страницы продукта, а не экзаменационный invariant: перед покупкой/активацией сверяйте их с текущим LF ExamUI и инструкциями. Активируйте попытку только когда сможете использовать это окно целиком.
...
> 🧠 До правки определите asset, слой настройки, identity/node/namespace/context, разрешённый и запрещённый результат и наблюдаемое доказательство.
...
> 🏭 Экзаменационный цикл «scope → минимальная обратимая правка → evidence → проверка» становится incident-дисциплиной, если дополнить его change record, peer review, rollback plan и защитой доступности сервиса.
...
| [Лаба 114](../../labs/114/README_RU.MD) | kubeconfig contexts, извлечение client certificate, снижение exposure Service NodePort → ClusterIP |
| [Лаба 115](../../labs/115/README_RU.MD) | Cilium с нуля: замена kube-proxy, WireGuard, Mutual Authentication на SPIRE (advanced/production, не CKS Core) |
