[← Оглавление курса](README_RU.md) · [Путеводитель CKS](CKS_RU.md) · [Глоссарий](GLOSSARY_RU.md) · [Шпаргалка](CHEATSHEET_RU.md)

# Справочник ошибок и дебага CKS

Формат «Симптом → Причина → Быстрое решение» для самых частых причин `[FAIL]` в лабах
`101-112`. Все записи основаны на реальных `HINT:`-строках, которые `check_result` уже
...
└── Причина 2: удалена только `ClusterRoleBinding`, но не сама `ClusterRole` - при пересоздании роли с тем же именем права возвращаются.
    └── Решение: удаляйте и Role/ClusterRole, и Binding вместе, а не только связь. *(лаба 104, задание 4)*
...
**[Симптом] `kubectl` внезапно возвращает 401 на все запросы после правки `kube-apiserver.yaml`**
├── Причина: добавлен `--anonymous-auth=false`, но static Pod ещё не перезапустился с новым манифестом.
│   └── Решение: подождите, пока kubelet заметит изменение файла и пересоздаст static Pod - редактирование самого `Pod`-объекта через API не поможет, kubelet пересоздаст его из манифеста на диске. *(лаба 104, задание 5)*
...
**[Симптом] `cosign verify` для образа с ключом не проходит, хотя образ подписан**
├── Причина: verify выполнен для другого digest/тега, не того, что был подписан (например, после пересборки без изменений digest совпал с уже подписанным).
│   └── Решение: явно сравните `$SIGNED_DIGEST`/`$UNSIGNED_DIGEST` перед verify; для гарантированно другого artifact используйте `--build-arg` с уникальным значением. *(лаба 111, задание 9c)*
