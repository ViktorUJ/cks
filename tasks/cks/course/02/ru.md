...
> **Проблема.** Защита одного слоя Kubernetes создаёт ложное чувство безопасности:
> NetworkPolicy не исправляет публичный API, а hardened container не закрывает уязвимость
> в коде или облачные credentials ноды. Без карты активов и границ команда закрывает
> привычные настройки, оставляя атакующему более слабый путь через Cloud, Cluster,
> Container или Code.
