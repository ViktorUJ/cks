...
Остальное на этом слое требует CLI провайдера и отдельных прав в его аккаунте - `kubectl` эти объекты не видит в принципе. Вопросы одинаковы у всех провайдеров, различаются только команды:
...
> Это пример того, как такую проверку организуют в реальном production-окружении с тысячами Pod, а не материал, нужный для экзамена CKS. На экзамене хватит идеи и простой версии из шага 4 выше - там нет кластеров такого масштаба, и время лучше не тратить на chunking, temp-файлы и progress reporting. Раздел ниже - для тех, кто хочет увидеть, как этот же принцип выглядит на реальном проде, и не обязателен для подготовки к самому экзамену.
