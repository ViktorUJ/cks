...
> **Проблема.** Недоверенный tenant, CI-job или пользовательский plugin в обычном контейнере
> использует то же ядро ноды, что и kubelet и соседние Pod. Уязвимость kernel/runtime или
> неверно оставленная привилегия может превратить выполнение кода в container escape и доступ
> к хосту либо другим арендаторам. Sandboxed runtime добавляет отдельную границу между таким
> workload и ядром, не ослабляя остальные политики Pod.
