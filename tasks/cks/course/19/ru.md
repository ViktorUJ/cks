...
**OS-specific правило.** Начиная с PSS v1.25 для Pod с `.spec.os.name: windows`
ограничения Linux по privilege escalation, seccomp и capabilities не применяются. Не
требуйте от Windows Pod `allowPrivilegeEscalation: false`, `seccompProfile` или `drop: ALL`
так же, как от Linux Pod; Windows HostProcess и остальные применимые Windows-controls
