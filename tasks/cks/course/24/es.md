...
```text
registry.example.com/payments/api:1.4.2@sha256:<digest-verificado-de-64-caracteres>
```
...
# Dockerfile
FROM golang:1.27.1-alpine3.24@sha256:<digest-verificado> AS builder
WORKDIR /src
...
```dockerfile
FROM gcr.io/distroless/static-debian13:nonroot@sha256:<digest-verificado>
COPY --from=builder /out/server /server
USER 65532:65532
ENTRYPOINT ["/server"]
```
...
```dockerfile
# syntax=docker/dockerfile:1.7
FROM node:22.23.2-alpine@sha256:<digest-verificado> AS builder
WORKDIR /app
