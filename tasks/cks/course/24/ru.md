...
> **Проблема.** После RCE полный runtime-образ даёт атакующему не только процесс
> приложения, но и shell, package manager, компилятор, исходники и лишние библиотеки.
> Каждый такой компонент добавляет CVE или готовый инструмент для скачивания payload,
> разведки и закрепления. Если builder целиком попадает в final image, риск повторяется
> на каждой ноде, куда этот artifact будет скачан и запущен.
