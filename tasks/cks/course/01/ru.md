...
> **Проблема.** Kubernetes-кластер может выглядеть работоспособным для администратора CKA,
> но оставаться незащищённым: отдельные решения по сети, RBAC, образам и журналам не
> складываются в защиту без модели угроз и проверки результата. Эта глава задаёт карту
> доменов, prerequisites и инструментов, чтобы следующие меры hardening были частью
> defense in depth, а не набором несвязанных команд.
...
> 🎯 Подтвердите правила и версию попытки, ориентируйтесь в curriculum, prerequisite CKA и инструментах по слоям.

## 01.2 Формат экзамена, версия и документация

Экзамен CKS performance-based: практические задачи выполняются в терминале на предоставленных кластерах и нодах. Отводится 2 часа, проходной балл - 67%. На момент проверки Important Instructions ориентируют на **15–20 практических задач**; это параметр снимка, который Linux Foundation может изменить. Для регистрации и сдачи CKS требуется ранее сданный CKA, но срок его действия к моменту CKS может истечь: сертификат CKA не обязан оставаться активным. Рабочая модель подготовки - осознанно переключать context и после каждого изменения проверять фактическое состояние.

Задача может назначить отдельный host: в этом случае выполните `ssh <host>` с базовой машины (`base`), выполните работу и вернитесь на `base`. Вложенный SSH между целевыми host не поддерживается. Набор предустановленных инструментов на `base` и на целевом host может различаться, поэтому сначала смотрите, где именно требуется выполнить команду. **Стандартная регистрация CKS** включает две реальные попытки экзамена (**One Retake**) в eligibility window **12 месяцев**; полученный сертификат действует **2 года**. Это не попытки simulator: стандартная регистрация также включает две попытки Killer.sh simulator, каждая активируется на **36 часов** и содержит **17 вопросов**; **CKS-SINGLE доступа к simulator не включает**. Тренируйте полный цикл: прочитать условие, выбрать host/context, внести минимальную правку и проверить результат.
...
| Новые поля и admission-поведение | Полезны для обучения, но не являются обещанием экзамена | Используйте только API и поведение версии, указанной в среде |
...
Изменения программы от **15 октября 2024** вступили в силу в эту дату. Действующие веса ниже берутся с Linux Foundation; публичный CNCF curriculum-репозиторий может всё ещё показывать прежние `10% / 15% / 15%`, поэтому не используйте его как источник текущих весов. Вес домена - ориентир для распределения времени, но не замена проверке всех компетенций.
...
> 🏭 Threat model, versioned policy и hardening, CI-проверки, наблюдаемое применение и пересматриваемые исключения.
