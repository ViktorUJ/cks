...
Внутри глав встречаются четыре визуальных маркера, которые разделяют материал по типу, а не по важности:

- 🎯 **CKS Core** - нужно уметь выполнить и проверить на экзамене.
- 🧠 **Почему это работает** - модель механизма, объясняет reasoning.
...
| Домен                                | Лабы                                                                                                                                                                                                            |
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🟦 Cluster Setup                          | [101](../labs/101/README_RU.MD) NetworkPolicy и metadata, [102](../labs/102/README_RU.MD) Cilium L3/L4/L7, [103](../labs/103/README_RU.MD) CIS, TLS и binary verification, [115](../labs/115/README_RU.MD) Cilium bootstrap и kube-proxy replacement (advanced/production, не CKS Core)                                            |
| 🟥 Cluster Hardening                      | [104](../labs/104/README_RU.MD) RBAC, ServiceAccount и API access, [113](../labs/113/README_RU.MD) kubeadm upgrade, [114](../labs/114/README_RU.MD) kubeconfig contexts, client certificate и Service exposure                                                                                                 |
| 🟧 System Hardening                       | [105](../labs/105/README_RU.MD) ОС, сеть и Docker daemon, [106](../labs/106/README_RU.MD) AppArmor и seccomp                                                                                                  |
| 🟩 Minimize Microservice Vulnerabilities  | [107](../labs/107/README_RU.MD) PSA и SecurityContext, [108](../labs/108/README_RU.MD) admission policies, [109](../labs/109/README_RU.MD) encryption at rest, [110](../labs/110/README_RU.MD) gVisor, Cilium и Istio, [115](../labs/115/README_RU.MD) WireGuard и Cilium Mutual Authentication на SPIRE (advanced/production, не CKS Core) |
| 🟪 Supply Chain Security                  | [108](../labs/108/README_RU.MD) allowlist, [111](../labs/111/README_RU.MD) images, SBOM, scan, signing и multi-image CVE triage                                                                                                              |
| 🟨 Monitoring, Logging & Runtime Security | [112](../labs/112/README_RU.MD) Falco, audit-логи и иммутабельность                                                                                                                              |

## Практика

У курса четыре уровня практики, и они не заменяют друг друга - каждый проверяет свой навык, от быстрой проверки одного факта (Level 1) до независимой валидации перед экзаменом (Level 4):
...
- ⚡ **Level 1** (5-15 минут). Killercoda-сценарии в большинстве глав (например, `rbac-serviceaccount-permissions`) - быстрая проверка одного факта или команды сразу после теории.
- 🔬 **Level 2** (30-120+ минут). 🧪 [Лабораторные работы CKS](../labs) - план из 13 лабораторных работ с автоматической проверкой `check_result`, от NetworkPolicy до Falco, audit-логов и kubeadm upgrade. Здесь вырабатывается полный workflow: hardening → break → verify → evidence.

> **Почему эталонные решения короткие.** У одной лабораторной задачи может быть несколько технически корректных решений. Эталонные solutions курса не претендуют на единственный правильный способ: они намеренно выбирают короткий, повторяемый и легко проверяемый путь, который помогает минимизировать время и количество действий при выполнении похожих задач на экзамене. Цель solution — выработать экзаменационную мышечную память: быстро выполнить требуемое изменение и сразу убедиться, что результат действительно корректен. Более универсальные или production-oriented варианты могут быть полезны в реальной эксплуатации, но не являются целью exam-oriented solution.
- 🎯 **Level 3** (120 минут). 🧪 [Мок-экзамены CKS](../mock) - репетиции под таймером, смешивающие все домены сразу; на английском, как и задания реального экзамена (LF предоставляет CKS также на японском и Simplified Chinese через отдельную регистрацию, но не на русском) - привыкайте читать формулировки заданий на английском заранее.
- 🧭 **Level 4** (независимая среда). [Killer.sh](https://killer.sh/cks) (входит в стандартную регистрацию на экзамен LF) - два симулированных прогона по 17 заданий в отдельном 36-часовом окне каждый. Используйте его в конце подготовки, а не вместо Level 2-3: это финальный стресс-тест, а не основной источник знаний. **Важно:** доступ к симулятору не включён в регистрацию `CKS-SINGLE` (экзамен без ретейка) - если вы регистрировались по этому тарифу, Killer.sh нужно будет купить отдельно на сайте Killer.sh, либо ориентироваться только на Level 2-3.
