...
@test "0  Init  " {
  echo ''>/var/work/tests/result/timed_all
  echo ''>/var/work/tests/result/timed_ok
  [ "$?" -eq 0 ]

}

@test "1.1  Container Runtime Sandbox gVisor.RuntimeClass " {
  echo '1'>>/var/work/tests/result/timed_all
  ...
  if [[ "$result" == "runsc" ]]; then
   echo '1'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "runsc" ]
}
...
@test "1.3  Container Runtime Sandbox gVisor. deployment1 nodeSelector " {
  ...
  if [[ "$result" == "runsc" ]]; then
   echo '.3'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "runsc" ]
}
...
@test "1.9  Container Runtime Sandbox gVisor. logs dmesg  " {
  echo '.7'>>/var/work/tests/result/timed_all
  cat /var/work/tests/artifacts/1/gvisor-dmesg | grep 'Starting gVisor'
  result=$?
  if [[ "$result" == "0" ]]; then
   echo '.7'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "0" ]
}
...
@test "4.1 CIS Benchmark. check 1.2.15  " {
  echo '.75'>>/var/work/tests/result/timed_all
  ...
  result=$?
  if [[ "$result" == "0" ]]; then
   echo '.75'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "0" ]
}
...
@test "6.1 set tls version  and  allowed ciphers.etcd cipher " {
  echo '2'>>/var/work/tests/result/timed_all
  ...
  result=$?
  if [[ "$result" == "0" ]]; then
   echo '2'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "0" ]
}
...
@test "14 fix Dockerfile " {
  echo '4'>>/var/work/tests/result/timed_all
  podman  run -d --name  cks-14   cks:14
  sleep 2
  ...
  podman rm  cks-14
  if [ "$result" == "0" ]; then
   echo '4'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "0" ]
}

@test "15 Pod Security Standard" {
  echo '6'>>/var/work/tests/result/timed_all
  cat /var/work/tests/artifacts/15/logs | grep FailedCreate | grep forbidden
  result=$?
  set -e
  if  [ "$result" == "0" ]; then
   echo '6'>>/var/work/tests/result/timed_ok
  fi
  [ "$result" == "0" ]
}
