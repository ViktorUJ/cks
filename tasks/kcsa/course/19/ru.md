...
```mermaid
flowchart TB
    req["требование<br/>или риск"] --> control["технический<br/>контроль"]
    control --> evidence["доказательство:<br/>лог, отчёт, provenance"]
    evidence --> review["проверка<br/>и улучшение"]
    review --> req
...
| SOC 2 | аудиторская оценка controls сервисной организации по Trust Services Criteria | Type I: suitability of control design на указанную дату; Type II: design и operating effectiveness controls за заявленный период | доступ по ролям, change management, мониторинг, evidence из CI/CD |
...
flowchart TB
    ...
    admission["admission<br/>и подпись"] -. "предотвращают" .-> execute
    audit["audit и runtime<br/>signals"] -. "обнаруживают" .-> persist
...
**Reproducible build** — сборка, при которой при тех же исходниках, определённой build environment и тех же build instructions независимая сторона может воспроизвести bit-for-bit идентичные указанные артефакты. Reproducibility помогает независимо сверять соответствие source → artifact, но сама по себе не доказывает доверенную signing identity, не заменяет provenance и не определяет уровень SLSA Build или Source.
...
Compliance evidence должно сохранять время проверки, scope, версию tool/policy и идентификатор проверяемой среды или артефакта. Доступ к evidence ограничивают от несанкционированного изменения; для повышенной assurance используют append-only, immutable или tamper-evident хранение. Иначе позднее невозможно надёжно доказать, что сохранённый результат соответствует реально выполненной проверке.
...
### 5. Какое evidence корректно описывает SLSA v1.2 в отчёте о supply chain?
   ...
   - a. Указать наличие подписи и считать её заменой provenance, SBOM, результатов scan и отдельного объявления применимого SLSA track.

   - b. Указать применимый Build или Source track и его уровень, а связанные evidence сохранять отдельно согласно назначению каждого вида доказательств.

   - c. Указать наличие SBOM и на его основании присвоить одинаковый SLSA level одновременно Build и Source tracks без дополнительного evidence.

   - d. Указать reproducible build и использовать его как универсальный SLSA level независимо от выбранного track, provenance и требований уровня.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: b.** SLSA v1.2 имеет отдельные Build и Source tracks со своими уровнями и требованиями. Поэтому уровень указывают вместе с конкретным track.

SBOM, signature, provenance и результаты scan отвечают на разные вопросы и не становятся взаимозаменяемыми только из-за использования SLSA. Reproducible build также не является универсальным обозначением SLSA level.

</details>

> **Куда дальше.** Для практической проверки CIS Benchmark используйте главу 07 CKS. Сценарии admission control разобраны в главе 20 CKS; supply chain, SBOM, подписи и policy - в главах 25-28 CKS. Для настройки и анализа audit logging используйте главу 32 CKS.
