...
KCSA проверяет концептуальное понимание безопасности cloud native и Kubernetes. Это online proctored экзамен с вопросами multiple choice, а не практическая работа в командной строке. **По правилам Linux Foundation, проверенным 1 сентября 2026 года, стандартный MCQ-экзамен (multiple choice question, вопрос с выбором ответа) содержит 60 вопросов, длится 90 минут и требует 75% для прохождения.**
...
Число вопросов, длительность, проходной балл и остальные организационные условия могут измениться после даты снимка. Перед регистрацией перепроверяйте актуальные материалы Linux Foundation, а не старый блог, пересказ курса или тренировочный тест.
...
При 60 вопросах за 90 минут средний бюджет составляет **1,5 минуты на вопрос**. Это не обязанность отвечать ровно за 90 секунд: простые вопросы создают резерв для сценариев, таблиц и неоднозначных формулировок.

```mermaid
flowchart TB
    read["прочитать<br/>вопрос"] --> decide{"ответ<br/>понятен?"}
    decide -->|"да"| answer["выбрать ответ<br/>и двигаться дальше"]
    decide -->|"нет"| eliminate["исключить явно неверное<br/>и пометить вопрос"]
    eliminate --> continue["пройти остальные<br/>вопросы"]
    answer --> review["вернуться к помеченным<br/>с резервом времени"]
    continue --> review
    review --> final["проверить ответы<br/>и отправить экзамен"]
...
- На снимке 2026-09-01 KCSA следует стандартному LF MCQ-формату: 60 вопросов, 90 минут, проходной балл 75%; экзамен проходит online с proctoring.
- Число вопросов, длительность, проходной балл и остальные организационные условия перед попыткой нужно перепроверять в актуальных материалах Linux Foundation.
...
### 2. Как лучше поступить с вопросом, на который после разумного исключения вариантов всё ещё нет уверенного ответа?
   ...
   - a. Оставить вопрос без ответа и завершить попытку сразу, чтобы не рисковать неправильным выбором.

   - b. Выбрать наиболее обоснованный вариант, пометить вопрос и вернуться к нему после первого прохода.

   - c. Изменить предыдущие ответы при первом сомнительном вопросе, даже если для них были уверенные основания.

   - d. Остановиться на этом вопросе и потратить всё оставшееся время до появления полной уверенности.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: b.** При ограниченном времени полезно сохранить темп первого прохода и затем вернуться к отмеченным вопросам. Конкретные возможности интерфейса экзамена необходимо проверить перед сессией.
...
**Мок-экзамены KCSA:** [Mock Exam 01](../../mock/01/README.md) · [Mock Exam 02](../../mock/02/README.md) - 60 вопросов каждый, closed-book, 90 минут (см. §20.5).
