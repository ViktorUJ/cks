...
flowchart LR
    ...
    decide -->|"да"| answer["выбрать ответ\nи двигаться дальше"]
    decide -->|"нет"| eliminate["исключить явно неверное\nи пометить вопрос"]
    eliminate --> continue["пройти остальные вопросы"]
    answer --> review["вернуться к помеченным\nс резервом времени"]
    continue --> review
    review --> final["проверить выбранные ответы\nи отправить экзамен"]
...
> **Куда дальше.** После KCSA углубите административную практику в [курсе CKA](../../../cka/course/README_RU.md). Linux Foundation требует сданный CKA перед попыткой CKS; [курс CKS](../../../cks/course/README_RU.md) можно использовать как дополнительное чтение, но он не заменяет этот prerequisite.
