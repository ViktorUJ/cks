...
```mermaid
flowchart TB
    C["Клиент: kubectl,<br/>controller,<br/>приложение"] -->|"TLS-запрос"| A["kube-apiserver"]
    A -->|"чтение и<br/>запись состояния"| E[("etcd")]
    M["kube-controller-<br/>manager"] <-->|"Kubernetes API"| A
    S["kube-scheduler"] <-->|"Kubernetes API"| A
    A -->|"назначенный Pod"| K["kubelet на<br/>рабочем узле"]
...
flowchart TB
    ...
    AU --> AZ["Authorization<br/>Разрешено ли<br/>действие?"]
    AZ --> AD["Admission<br/>Соответствует ли<br/>объект политике?"]
    AD --> API["Обработка<br/>API-запроса"]
    API --> ETCD[("etcd")]
...
Порядок важен для MCQ (multiple choice question, вопрос с выбором ответа): admission не заменяет authentication и не назначает пользователю права. Он получает уже аутентифицированный и авторизованный запрос.
...
Для mTLS важно разделять роли сертификатов. Например, PKI, создаваемая `kubeadm`, использует отдельный `etcd-ca` для etcd-related trust и отдельный клиентский сертификат `apiserver-etcd-client`, которым `kube-apiserver` аутентифицируется перед `etcd`. Это не означает, что любая установка Kubernetes обязана иметь именно такую файловую структуру или отдельный root CA, но разделение trust domains / CA chains позволяет не смешивать serving- и client-credentials разных компонентов, отдельно ограничивать доверие и независимо планировать rotation или migration etcd.

Нельзя использовать server certificate `kube-apiserver` как универсальный shared credential для etcd. Сертификат должен соответствовать своей роли, а private keys и CA material защищают как чувствительные control-plane credentials.
...
## 07.4 Controller Manager и Scheduler: служебные идентичности (service identity) и поверхность атаки
...
Флаг сам по себе не всегда является уязвимостью. Например, endpoint metrics может быть нужен системе мониторинга. Вопрос безопасности звучит так: кто может к нему подключиться, как аутентифицируется этот субъект, что он может узнать или изменить и есть ли менее рискованный способ предоставить нужную функцию.
...
| служебная идентичность (service identity) | Учётная запись компонента, с которой он обращается к Kubernetes API. |
...
### 5. Как следует относиться к credentials `kube-controller-manager` и `kube-scheduler`?
   ...
   - a. Как к общим административным credentials, если control-plane endpoint закрыт внутренней сетью.

   - b. Как к публичным служебным данным, потому что эти компоненты работают внутри control plane.

   - c. Как к привилегированным API credentials компонентов, которые защищают и ограничивают по least privilege.

   - d. Как к замене serving certificate API Server, если между компонентами уже используется TLS.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: c.** `kube-controller-manager` и `kube-scheduler` являются аутентифицированными API-клиентами. Их kubeconfig, client certificates, keys или tokens являются чувствительными credentials и должны иметь только необходимые компоненту permissions. Внутренняя сеть не делает shared admin credentials безопасными, а client identity компонента не заменяет serving certificate API Server.
