...
```mermaid
flowchart LR
    dev["Разработчик\nкод и манифест"] --> ci["CI/CD\nсборка и проверка"]
    ci --> registry["Registry\nобраз"]
    registry --> cluster["Kubernetes\nоркестрация"]
    cluster --> workload["Рабочая нагрузка\nв контейнере"]
...
```mermaid
flowchart LR
    vuln["Уязвимость\nприложения"] --> token["Токен или\nучётные данные"]
    token --> api["Kubernetes API"]
    api --> pod["Опасный Pod"]
    pod --> node["Рабочий узел"]
    guard1["Исправление кода\nи защита секретов"] -. "снижает риск" .-> token
    guard2["Least privilege\nи admission"] -. "ограничивает" .-> pod
    guard3["Hardening узла\nи изоляция"] -. "сдерживает" .-> node
...
- Контейнер - не полная граница безопасности: существенны права, сеть, токены, узел и происхождение артефакта.
...
Ловушка MCQ - выбрать один сильный инструмент как замену всем остальным. Сканер образа, RBAC и шифрование решают разные части задачи и обычно дополняют друг друга.
...
### 3. Разработчику нужен доступ только на чтение `ConfigMap` в одном `Namespace`. Какое решение соответствует least privilege?
   ...
   a. Выдать `cluster-admin` через `ClusterRoleBinding`.
   b. Отключить аутентификацию для внутренней сети.
   c. Создать `Role` с `get` и `list` для `ConfigMap` в нужном `Namespace` и связать её с identity.
   d. Разрешить контейнеру все Linux capabilities.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: c.** Права ограничены ресурсом, действиями и областью `Namespace`. Остальные варианты расширяют доступ или не относятся к требованию.
...
> **Куда дальше.** Модель слоёв Cloud, Cluster, Container и Code разобрана в [главе 02 CKS](../../../cks/course/02/ru.md) на практическом уровне. В этом курсе продолжайте с [главы 03](../03/ru.md), где 4C показаны как единая модель cloud native security.
