...
В Kubernetes нет политики, которая сама по себе запрещала бы весь трафик. Если `Pod` не выбран политикой, его входящий и исходящий трафик обычно разрешён. Это часто называют default allow. В плоской сети скомпрометированный `Pod` может искать базы данных, внутренние API и сервисы других команд.

```mermaid
flowchart LR
    bad["Скомпрометированный\nPod frontend"] --> scan["Сканирует сервисы\nи порты"]
    scan --> db["База данных"]
    scan --> api["Внутренний API"]
    deny["Default-deny"] --> allow["Явные разрешения\nтолько для нужных связей"]
...
**Default-deny** означает, что для направления трафика сначала создают запрет по умолчанию, а затем добавляют узкие allow-политики. Важна точность формулировки: политика становится изолированной отдельно для `Ingress` и `Egress`, когда она выбирает `Pod` и объявляет соответствующее направление в `policyTypes`. Политики аддитивны: соединение разрешено, если его допускает хотя бы одно подходящее правило. Политика не создаёт отрицательных правил с приоритетом «запретить поверх разрешения».
...
a. Разрешён только трафик из того же `Namespace`.

b. Трафик обычно разрешён, пока его не ограничивает применимая политика и её CNI-enforcement.

c. Разрешён только DNS-трафик.

d. Входящий и исходящий трафик всегда запрещён Kubernetes.
...
a. Изолирует все выбранные `Pod` от входящего и исходящего трафика, пока другие политики не добавят разрешения.

b. Запрещает доступ к Kubernetes API для пользователей.

c. Разрешает весь трафик только между `Pod` этого namespace.

d. Удаляет все `Pod` из namespace.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: a.** Пустой `podSelector` выбирает все `Pod` namespace, а пустые списки правил не создают разрешений. Дополнительная подходящая allow-политика может разрешить конкретный путь, потому что политики аддитивны.
...
a. Ограничить HTTP-доступ конкретным методом и путём или разрешить egress по DNS-имени.

b. Выбрать все `Pod` namespace пустым `podSelector`.

c. Выбрать `Pod` по label.

d. Разрешить TCP-порт 443.

<details>
<summary>Ответ и разбор</summary>

**Верный ответ: a.** Базовая `NetworkPolicy` покрывает сетевые идентичности и порты L3/L4. Cilium может добавить L7 HTTP-контроль и DNS-aware правила, если эти возможности включены и используются в его политике.

</details>

> **Куда дальше.** Для практического проектирования default-deny и allow-политик изучите [главу 04 CKS о `NetworkPolicy`](../../../cks/course/04/ru.md). Защита metadata-сервисов и служебных endpoints разобрана в [главе 05 CKS](../../../cks/course/05/ru.md), а L3/L4/L7 и DNS-aware политики Cilium - в [главе 06 CKS](../../../cks/course/06/ru.md). Для административной основы сети `Pod` и CNI полезна [глава 34 CKA](../../../cka/course/34/ru.md).
