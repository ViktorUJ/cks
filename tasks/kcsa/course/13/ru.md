...
L7 и DNS-aware политики не являются возможностями базового API `NetworkPolicy`; они зависят от Cilium и его конфигурации. L7-контроль не уникален для Cilium: он реализует его на уровне CNI через eBPF без sidecar-proxy, а сервис-меши (Istio, Linkerd) достигают похожего результата на уровне приложения через sidecar-proxy, попутно добавляя mTLS и telemetry (см. главу 18 о PKI, mTLS и service mesh). L7-политики CNI и service mesh не заменяют проверку приложения: разрешение `GET /healthz` на L7 полезнее доступа ко всему HTTP-сервису, но не исправит уязвимость сервера. Cilium также даёт наблюдаемость сетевых решений, что помогает понять, почему соединение разрешено или отклонено.
...
Шифрование трафика между `Pod` - отдельная задача от `NetworkPolicy` и L7-фильтрации CNI: она решается через TLS/mTLS на уровне приложения или через service mesh (например Istio, Linkerd), который добавляет sidecar-proxy, workload identity и mTLS без изменения кода приложения (подробнее в главе 18). `NetworkPolicy` и Cilium L7-политики могут разрешить или запретить соединение, но не делают его содержимое конфиденциальным.
