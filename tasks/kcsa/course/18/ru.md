...
```mermaid
flowchart TB
    ca["cluster CA\nкорень доверия"] --> apiCert["сертификат API Server"]
    ca --> clientCert["клиентские сертификаты\nадминистратора и компонентов"]
    ca --> kubeletCert["сертификат kubelet"]
    kubeconfig["kubeconfig\nserver + CA + credentials"] --> api["API Server"]
    clientCert --> api
    apiCert --> api
...
**Service mesh** добавляет уровень управления сервисным трафиком. Istio и Linkerd обычно используют прокси рядом с приложением или на уровне узла. Прокси берёт на себя установление mTLS, выдачу и ротацию короткоживущих identity, наблюдаемость и применение политик. Приложению не обязательно реализовывать TLS для каждого внутреннего вызова самостоятельно.
...
flowchart LR
    ...
    policy["политика трафика\nallow, timeout, retry"] -. "применяется прокси" .-> cp
...
> **Куда дальше.** Практическое шифрование Pod-to-Pod трафика и mTLS в Cilium, Istio и Linkerd разобраны в [главе 23 CKS](../../../cks/course/23/ru.md). Настройка и проверка runtime detection Falco - в [главе 29 CKS](../../../cks/course/29/ru.md).
