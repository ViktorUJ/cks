...
```mermaid
flowchart TD
    client["Разработчик, CI<br/>или контроллер<br/>с клиентскими credentials"]
    api["API Server<br/>authentication,<br/>authorization, admission"]
    etcd[("etcd<br/>состояние кластера<br/>и Secret")]
    kubelet["kubelet<br/>рабочий узел"]
    runtime["container runtime<br/>containerd или CRI-O"]
    pod["Pod<br/>процессы и данные<br/>приложения"]
    net["внешняя сеть, Service<br/>или другой Pod"]

    client <-->|"TLS: запрос<br/>к API"| api
    api <-->|"TLS: чтение<br/>и запись состояния"| etcd
    api <-->|"TLS: desired state<br/>и статус"| kubelet
    kubelet <-->|"CRI на узле"| runtime
    runtime <-->|"процессы, filesystem,<br/>namespaces"| pod
...
Фреймворки не являются взаимозаменяемыми списками настроек: у каждого своя область применения и свой вопрос, на который он отвечает. Сначала - обзор, что решает каждый, затем детальный разбор STRIDE и ATT&CK for Containers по отдельности.

| Фреймворк | На какой вопрос отвечает | Единица анализа | Когда применять |
|---|---|---|---|
| STRIDE | Какие классы угроз возможны у конкретного потока или границы? | элемент архитектуры (компонент, поток данных, доверенная граница) | на этапе проектирования или ревью архитектуры, до инцидента |
| MITRE ATT&CK for Containers | Какие тактики и техники атакующий уже применяет или может применить в контейнерной среде? | наблюдаемое поведение атакующего (тактика → техника) | при построении детекта, разборе инцидента, оценке покрытия runtime-защиты |
| Kill chain | На каком этапе развития атаки её эффективнее остановить? | последовательность этапов одной атаки (от подготовки до цели) | при выборе, где расположить preventive и detective control относительно друг друга |

**STRIDE** и **ATT&CK for Containers** не конкурируют, а покрывают разные стороны одной картины: STRIDE - это анализ архитектуры "от угрозы", применяемый заранее, ATT&CK - это анализ поведения "от атакующего", применяемый к уже наблюдаемым или гипотетическим действиям. **Kill chain** - это не отдельный список угроз или техник, а способ упорядочить результат STRIDE и ATT&CK по времени: он показывает, на каком этапе конкретная угроза (из STRIDE) или техника (из ATT&CK) фактически проявится, и помогает решить, где выгоднее поставить preventive control, а где - detective.

**Best practice сочетания.** Не пытайтесь свести все три фреймворка к одному документу или одной таблице - у них разные оси анализа, и принудительное объединение размывает вопрос, на который отвечает каждый. Практичный порядок: (1) для новой архитектуры или существенного изменения сначала пройти STRIDE по каждому элементу и потоку - это даёт список угроз и trust boundaries; (2) для угроз, которые реалистичны в вашей среде, сопоставить их с тактиками и техниками ATT&CK for Containers - это даёт конкретные наблюдаемые сигналы и existing detection coverage; (3) разложить результат по kill chain, чтобы увидеть, какие этапы атаки покрыты preventive control, какие - только detective, и где есть разрыв. STRIDE и ATT&CK не обязаны совпадать один-к-одному: одна угроза STRIDE (например Elevation of Privilege) может проявляться через несколько техник ATT&CK (privileged container, hostPath, capability abuse), и это ожидаемо, а не ошибка анализа. Подробное сопоставление с фреймворками и комплаенсом дано в главе 19.
...
В MCQ (multiple choice question, вопрос с выбором ответа) часто описывают один компонент или сценарий и просят выбрать наиболее подходящий контроль. Сначала определите актив и границу: это API-доступ, данные etcd, права `Pod`, доступ к рабочему узлу или сетевой поток. Затем отделите профилактику от детекта и восстановления.
