...
`scratch` - пустая база. Она подходит не «для любого маленького образа», а для приложения, которое запускается без динамических библиотек и без отсутствующих файлов. Например, статический Go binary может потребовать CA certificate для TLS и настройки DNS. Если их не добавить осознанно, приложение перестанет работать. Безопасность не должна достигаться случайным удалением нужных компонентов.
...
**Верный ответ: d.** Разделение stage позволяет не включать build-инструменты, кеши и исходный код в runtime image. Подпись, анализ зависимостей и сканирование решают другие задачи.
